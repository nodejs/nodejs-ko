...
* **src**:
  * 메모리 한도를 얻으려고 cgroups를 사용합니다. 이는 Node.js 프로세스의 메모리 최댓값를 설정하는
    방법을 개선했습니다. 이전에는 필요한 V8 힙 크기를 추정하려고 물리 메모리를 사용했는데 프로세스가
    도커 컨테이너나 다른 제약 환경에서 돌아가는 경우 물리 메모리 크기는 정확한 한도가 아닙니다.
    이 변경으로 [도커 컨테이너에서 리소스 제약을 설정](https://docs.docker.com/config/containers/resource_constraints/)하기
    위해 linux cgroups로 설정된 메모리 한도를 가져올 수 있게 되었습니다.
    (Kelvin Jin) [#27508](https://github.com/nodejs/node/pull/27508)
