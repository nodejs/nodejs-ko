---
category: articles
title: Node v12.7.0(현재 버전)
author: Michaël Zasso
ref: Node v12.7.0 (Current)
refurl: https://nodejs.org/en/blog/release/v12.7.0
...
* **deps**:
  * nghttp2를 1.39.1로 업데이트했습니다. (gengjiawen) [#28448](https://github.com/nodejs/node/pull/28448)
  * npm을 6.10.0으로 업데이트했습니다. (isaacs) [#28525](https://github.com/nodejs/node/pull/28525)
* **esm**:
  * "pkg-exports" 제안을 실험적으로 구현했습니다. 모듈의 `package.json` 파일에 새로 구현된
    `"exports"` 필드를 추가하면 하위 경로의 별칭을 커스터마이징 해서 제공할 수 있습니다. 자세한
    내용은 [proposal-pkg-exports](https://github.com/jkrems/proposal-pkg-exports/)를
    참고하세요. (Guy Bedford) [#28568](https://github.com/nodejs/node/pull/28568)
* **http**:
  * `response.writableFinished`를 추가했습니다. (Robert Nagy) [#28681](https://github.com/nodejs/node/pull/28681)
  * `http.ClientRequest`의 `"information"` 이벤트에서 `headers`, `rawHeaders`
    및 그 외 필드를 노출합니다. (Austin Wright) [#28459](https://github.com/nodejs/node/pull/28459)
* **inspector**:
  * `inspector.waitForDebugger()`를 추가했습니다. (Aleksei Koziatinskii) [#28453](https://github.com/nodejs/node/pull/28453)
* **policy**:
  * 정책 조작을 줄일 수 있는 `--policy-integrity=sri` CLI 옵션을 추가했습니다. 정책 무결성을
    지정했는데 정책에 해당 무결성이 없다면 Node.js는 어떤 코드도 실행하기 전에 에러를 발생시킬
    것입니다. (Bradley Farias) [#28734](https://github.com/nodejs/node/pull/28734)
* **readline,tty**:
  * 문자를 작성하는 다양한 방법의 스트림 API를 노출했습니다.
    (cjihrig) [#28674](https://github.com/nodejs/node/pull/28674),
    [#28721](https://github.com/nodejs/node/pull/28721)
* **report**:
  * JSON 문자열 대신 `Object`를 반환하도록 `process.report.getReport()`를 수정했습니다.
    (Christopher Hiller) [#28630](https://github.com/nodejs/node/pull/28630)
* **src**:
  * 메모리 한도를 얻으려고 cgroups를 사용합니다. 이는 Node.js 프로세스의 메모리 최고치를 설정하는
    방법을 개선했습니다. 이전에는 필요한 V8 힙 크기를 추정하려고 물리 메모리를 사용했는데 프로세스가
    도커 컨테이너나 다른 제약 환경에서 돌아가는 경우 물리 메모리 크기는 정확한 한도가 아닙니다.
    이 변경으로 [docker containers to set resource constraints](https://docs.docker.com/config/containers/resource_constraints/)에서
    사용된 linux cgroups로 설정된 메모리 한도를 가져올 수 있게 되었습니다.
    (Kelvin Jin) [#27508](https://github.com/nodejs/node/pull/27508).
