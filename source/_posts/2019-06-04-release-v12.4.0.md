...
* **esm**:
  * `--experimental-modules`를 사용하면 JSON 모듈 지원이 항상 활성화됩니다.
...
* **inspector**:
  * 시작할 때 V8 힙 프로파일러를 켜고 종료하기 전에 디스크에 힙 프로파일 정보를 작성하는데 사용할 수 있는 `--heap-prof` 플래그가 실험적으로 추가되었습니다. (Joyee Cheung) [#27596](https://github.com/nodejs/node/pull/27596).
* **stream**:
