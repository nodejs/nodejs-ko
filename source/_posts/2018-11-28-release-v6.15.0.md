...
* Node.js: HTTP 요청 분열(CVE-2018-12116)
* OpenSSL: DSA 서명 생성 시 OpenSSL 시차 취약성(CVE-2018-0734)
* OpenSSL: ECC 스칼라 곱에서 OpenSSL의 마이크로아키텍처 시차 취약성(CVE-2018-5407)

### 주요 변경사항

* **debugger**: 디버거가 `0.0.0.0`에서 수신하지 않도록 [nodejs/node#8106](https://github.com/nodejs/node/pull/8106)를 백포트했습니다. Ben Noordhuis가 보고했습니다.(CVE-2018-12120 / Ben Noordhuis).
* **deps**: OpenSSL 1.0.2q로 업그레이드해서 CVE-2018-0734와 CVE-2018-5407을 수정했습니다.
* **http**:
    * 서비스 거부 공격 가능성을 차단하기 위해 HTTP 서버가 받은 전체 헤더가 8192가 넘지 않아야 합니다. Trevor Norris가 보고했습니다.(CVE-2018-12121 / Matteo Collina)
    * 이제 서버가 HTTP 헤더를 받을 때 40초의 시간 만료가 적용되었습니다. 이 값은 `server.headersTimeout`으로 조정할 수 있습니다.
    * 이제 서버가 HTTP 헤더를 받는데 40초의 만료 시간이 적용되었습니다. `server.headersTimeout`으로 이 값을 조정할 수 있습니다. 이 시간 내에 헤더를 완전히 받지 못했을 때는 다음 받는 청크에서 소켓이 소멸합니다. `server.setTimeout()`과 함께 사용하면 과도한 리소스 유지와 서비스 거부를 막는 데 도움이 됩니다. Jan Maybach([liebdich.com](https://liebdich.com))가 보고했습니다.(CVE-2018-12122 / Matteo Collina)
    * HTTP 클라이언트 요청의 `path` 옵션에서 두 바이트 문자는 이제 엄격하게 허용하지 않습니다. `\u0021` ~ `\u00ff` 범위 밖에 있는 문자를 담고 있는 경로는 이제 `TypeError`로 거절됩니다. 이 동작은 필요에 따라 `--security-revert=CVE-2018-12116` 명령행 인자를 제공하면 복구할 수 있습니다.(추천하지 않습니다.) [Arkadiy Tetelman](https://twitter.com/arkadiyt)([Lob](https://lob.com))가 Node.js 6과 8의 보안 이슈를 보고했고 Benno Fünfstück이 Node.js 10 이상에 적용된 변경사항을 배포트해서 수정했습니다.(CVE-2018-12116 / Matteo Collina)
* **url**: `'javascript:'` 프로토콜을 `url.parse()`로 URL 파싱할 때 호스트명이 스누핑될 수 있는 버그를 수정했습니다. [Martin Bajanik](https://twitter.com/_bayotop)([Kentico](https://kenticocloud.com/))가 보고했습니다.(CVE-2018-12123 / Matteo Collina)
