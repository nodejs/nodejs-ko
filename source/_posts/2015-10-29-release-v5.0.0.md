...
* **http**:
  ...
  - (호환성 깨짐) HTTP 메서드와 헤더 이름은 반드시 사용할 수 있는 문자, 사용할 수 없는 제어 문자, _구분자_를 나열한 RFC 2616 "토큰" 규칙을 준수해야 합니다. 특히 메서드와 헤더 이름은 반드시 ```/^[a-zA-Z0-9_!#$%&'*+.^`|~-]+$/```에 일치해야 합니다. 그렇지 않으면 `TypeError`가 발생합니다. (James M Snell) [#2526](https://github.com/nodejs/node/pull/2526)
* **node**:
