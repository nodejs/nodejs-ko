...
GCC 4.9.x 컴파일러로 업그레이드 중 스테이징을 잘못해서 PPC 리틀 엔디안을 위한 최신 릴리스에 GCC 4.8.x 대신 GCC 4.9.x를 사용했습니다. 이로인해 PPCLE 기반 환경에서 ABI 호환성이 깨졌습니다. 이 문제는 내부적으로 수정되었으며 이번 릴리스를 통해 제공하는 바이너리가 지원 대상 플랫폼에서 문제없이 동작하도록 하고자 합니다.
