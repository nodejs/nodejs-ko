...
대응되는 `AbortController`에서 `.abort()`를 호출하는 것은 자식 프로세스에서 `.kill()`을 호출하는 것과 비슷하지만 콜백에 전달한 에러가 `AbortError`가 될 것이다.
...
대응되는 `AbortController`에서 `.abort()`를 호출하는 것은 스트림에서 `.destroy(new AbortError())`를 호출하는 것과 똑같이 동작할 것이다.
