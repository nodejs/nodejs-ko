...
author: Myles Borins
ref: Node v6.9.3 (LTS)
refurl: https://nodejs.org/en/blog/release/v6.9.3
...
릴리스 이후에 v6.9.3을 위한 추가 플랫폼을 준비하는 동안 릴리스 서버의 타볼이
덮어 쓰여져 다른 shasum이 만들어졌습니다.  릴리스에 관한 모호성을 없애기 위해
변경이 없는 셈버 수 릴리스 v6.9.4를 만들었습니다.
...
* **deps**:
    - *npm*: npm을 3.10.10으로 업그래이드 했습니다. (Rebecca Turner) [#9847](https://github.com/nodejs/node/pull/9847)
    - *V8*: 계산된 속성에 화살표 함수의 디스트럭쳐링을 사용할 때 더 이상 예외를 발생시키지 않습니다. (Michaël Zasso) [#10386](https://github.com/nodejs/node/pull/10386)
* **inspector**: /json/version은 배열에 감싸진 객체가 아닌 객체를 반환합니다. (Ben Noordhuis) [#9762](https://github.com/nodejs/node/pull/9762)
* **module**: 이제 --debug-brk 와 --eval을 동시에 사용할 때 예상한 대로 동작합니다. (Kelvin Jin) [#8876](https://github.com/nodejs/node/pull/8876)
* **process**: nextTick의 성능을 최대 20%까지 향상시켰습니다. (Evan Lucas) [#8932](https://github.com/nodejs/node/pull/8932)
* **repl**:
    - 더 이상 나누기 연산자가 실수로 정규식으로 파싱되지 않습니다. (Teddy Katz) [#10103](https://github.com/nodejs/node/pull/10103)
    - 제너레이터 함수에 대한 지원을 강화했습니다. (Teddy Katz) [#9852](https://github.com/nodejs/node/pull/9852)
