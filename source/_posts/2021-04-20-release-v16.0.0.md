...
* **(SEMVER-MAJOR)** **fs**: rmdir의 recursive 옵션에서는 오류를 발생시키지 않는 동작을 제거했습니다. (Antoine du Hamel) [#37216](https://github.com/nodejs/node/pull/37216)
...
Timers Promises API는 Promise 개체를 반환하는 대신 timer 함수의 집합을 제공합니다. Node.js v15.0.0에 추가되었고, 이 릴리스에서 실험 상태를 벗어나 안정 버전으로 변경되었습니다.
...
Node.js v16.0.0은 애플 실리콘을 위해 미리 빌드한 바이너리를 제공하는 최초 릴리스가 될 것입니다. 우리는 Intel (`darwin-x64`)과 ARM (`darwin-arm64`) 아키텍처에 대해 분리된 타볼을  제공 할 예정이지만 맥OS 인스톨러(`.pkg`)는 ‘fat’ (multi-architecture) 바이너리로 제공될 예정입니다.

* **(SEMVER-MAJOR)** **build**: Python 2를 지원하지 않습니다. (Christian Clauss) [#36691](https://github.com/nodejs/node/pull/36691)
...
* **tools**: Distribution.xml에 아키텍처를 설정했습니다. (Ash Cripps) [#38261](https://github.com/nodejs/node/pull/38261)
...
이 업데이트는 또한 캡처된 문자열의 시작과 끝 인덱스를 제공하는 ECMAScript 정규 표현식 매치 인덱스를 포함합니다. 이 인덱스 배열은 정규 표현식이 `/d` 플래그가 있는 경우 매치된 객체에 `.indices` 속성을 통해 확인 가능합니다.
...
* **(SEMVER-MAJOR)** **lib,src**: 클러스터 모드에서 사용하던 단어 'Master'를 'Parent'로 변경했습니다. (Michael Dawson) [#36478](https://github.com/nodejs/node/pull/36478)
* **(SEMVER-MINOR)** **module**: `node:`로 시작하는 `require(…)` 호출을 지원합니다. (ExE Boss) [#37246](https://github.com/nodejs/node/pull/37246)
