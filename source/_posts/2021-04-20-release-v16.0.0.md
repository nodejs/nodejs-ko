---
category: articles
title: Node v16.0.0(현재 버전)
author: Bethany Nicolle Griggs
ref: Node v16.0.0 (Current)
refurl: https://nodejs.org/en/blog/release/v16.0.0
...
* **(SEMVER-MAJOR)** **fs**: 허가되었던 재귀적 rmdir이 제거되었습니다. (Antoine du Hamel) [#37216](https://github.com/nodejs/node/pull/37216)
* **(SEMVER-MAJOR)** **fs**: rmdir 재귀 옵션이 런타임 폐기 예정 상태가 되었습니다. (Antoine du Hamel) [#37302](https://github.com/nodejs/node/pull/37302)
* **(SEMVER-MAJOR)** **lib**: process.binding('http\_parser')에 접근하는 것은 런타임 폐기 예정 상태가 되었습니다. (James M Snell) [#37813](https://github.com/nodejs/node/pull/37813)
* **(SEMVER-MAJOR)** **lib**: process.binding('url')에 접근하는 것은 런타임 폐기 예정 상태가 되었습니다. (James M Snell) [#37799](https://github.com/nodejs/node/pull/37799)
* **(SEMVER-MAJOR)** **lib**: process.binding('util')은 오직 타입 체커를 반환하도록 만들었습니다. (Anna Henningsen) [#37819](https://github.com/nodejs/node/pull/37819)
* **(SEMVER-MAJOR)** **lib**: process.binding('crypto')에 접근하는 것은 런타임 폐기 예정 상태가 되었습니다. (James M Snell) [#37790](https://github.com/nodejs/node/pull/37790)
* **(SEMVER-MAJOR)** **lib**: process.binding('signal\_wrap')에 접근하는 것은 런타임 폐기 예정 상태가 되었습니다. (James M Snell) [#37800](https://github.com/nodejs/node/pull/37800)
* **(SEMVER-MAJOR)** **lib**: process.binding('v8')에 접근하는 것은 런타임 폐기 예정 상태가 되었습니다. (James M Snell) [#37789](https://github.com/nodejs/node/pull/37789)
* **(SEMVER-MAJOR)** **lib**: process.binding('async\_wrap')에 접근하는 것은 런타임 폐기 예정 상태가 되었습니다. (James M Snell) [#37576](https://github.com/nodejs/node/pull/37576)
* **(SEMVER-MAJOR)** **module**: module.createRequireFromPath는 제거되었습니다. (Antoine du Hamel) [#37201](https://github.com/nodejs/node/pull/37201)
* **(SEMVER-MAJOR)** **module**: 하위 경로 폴더 매핑은 런타임 폐기 예정 상태가 되었습니다. (Antoine du Hamel) [#37215](https://github.com/nodejs/node/pull/37215)
* **(SEMVER-MAJOR)** **module**: "main" 색인과 확장 조회는 런타임 폐기 예정 상태가 되었습니다. (Antoine du Hamel) [#37206](https://github.com/nodejs/node/pull/37206)
* **(SEMVER-MAJOR)** **module**: 유효하지 않은 package.json 메인 진입점은 런타임 폐기 예정 상태가 되었습니다. (Antoine du Hamel) [#37204](https://github.com/nodejs/node/pull/37204)
* **(SEMVER-MAJOR)** **process**: process.config 수정은 런타임 폐기 예정 상태가 되었습니다. (James M Snell) [#36902](https://github.com/nodejs/node/pull/36902)
...
Timers Promises API는 Promise 개체를 반환하는 timer 함수를 수정할 대안을 제공합니다. Node.js v15.0.0에 추가되었고, 이 릴리스에서 실험 상태를 벗어나 안정 버전으로 변경되었습니다.

James Snell이 기여했습니다. - [#38112](https://github.com/nodejs/node/pull/38112)
...
Node.js v16.0.0는 애플 실리콘을 위해 미리 빌드한 바이너리를 제공하는 최초 릴리스가 될 것입니다. 우리는 Intel (`darwin-x64`)와 ARM (`darwin-arm64`) 아키택쳐에 대해 분리된 tarballs를 제공 할 예정이지만 맥OS 인스톨러(`.pkg`)는 ‘fat’ (multi-architecture) 바이너리로 제공될 예정입니다.

* **(SEMVER-MAJOR)** **build**: Python 2 지원이 제거되었습니다. (Christian Clauss) [#36691](https://github.com/nodejs/node/pull/36691)
* **(SEMVER-MAJOR)** **build**: Makefile내 기본 PYTHON은 python3로 지정됩니다. (Michaël Zasso) [#37764](https://github.com/nodejs/node/pull/37764)
* **build**: Makefile이 fat 바이너리를 지원하기 위해 업데이트 됩니다. (Ash Cripps) [#37861](https://github.com/nodejs/node/pull/37861)
* **(SEMVER-MAJOR)** **build**: OS X에서 ASLR (PIE)이 가능합니다. (woodfairy) [#35704](https://github.com/nodejs/node/pull/35704)
* **build**: gcc 버전이 8.3.0보다 이전이면 경고합니다. (Richard Lau) [#37935](https://github.com/nodejs/node/pull/37935)
* **(SEMVER-MAJOR)** **doc**: Xcode 최소 지원을 11로 업데이트합니다. (Michaël Zasso) [#37872](https://github.com/nodejs/node/pull/37872)
* **(SEMVER-MAJOR)** **doc**: GCC 최소 지원을 8.3으로 업데이트합니다. (Michaël Zasso) [#37871](https://github.com/nodejs/node/pull/37871)
* **(SEMVER-MAJOR)** **doc**: AIX를 v16.x을 위한 GCC8로 업데이트합니다. (Ash Cripps) [#37677](https://github.com/nodejs/node/pull/37677)
* **tools**: Distribution.xml에 아키텍쳐를 설정했습니다. (Ash Cripps) [#38261](https://github.com/nodejs/node/pull/38261)
...
V8 자바스크립트 엔진은 성능 조정과 개선을 포함해 V8 9.0으로 업데이트 되었습니다.

이 업데이트는 또한 캡처된 문자열의 시작과 끝 인덱스를 제공하는 ECMAScript 정규 표현식 매치 인덱스를 포함합니다. 이 인덱스 배열은 정규 표현식이 `\d` 플래그를 가진 경우 매치된 객체에 `.indices` 속성을 통해 확인 가능합니다.

Michaël Zasso가 기여했습니다. - [#37587](https://github.com/nodejs/node/pull/37587)
...
* **(SEMVER-MINOR)** **assert**: assert.match와 assert.doesNotMatch가 실험 상태를 졸업했습니다. (James M Snell) [#38111](https://github.com/nodejs/node/pull/38111)
* **(SEMVER-MAJOR)** **buffer**: btoa와 atob는 전역으로 노출됩니다. (James M Snell) [#37786](https://github.com/nodejs/node/pull/37786)
* **(SEMVER-MAJOR)** **deps**: ICU 최소 버전이 68로 변경되었습니다. (Michaël Zasso) [#37330](https://github.com/nodejs/node/pull/37330)
* **deps**: ICU가 69.1로 업데이트되었습니다. (Michaël Zasso) [#38178](https://github.com/nodejs/node/pull/38178)
* **deps**: llhttp가 6.0.0으로 업데이트되었습니다. (Fedor Indutny) [#38277](https://github.com/nodejs/node/pull/38277)
* **deps**: npm의 버전을 7.10.0으로 업그레이드했습니다. (Ruy Adorno) [#38254](https://github.com/nodejs/node/pull/38254)
* **(SEMVER-MINOR)** **http**: http.ClientRequest.getRawHeaderNames()를 추가했습니다. (simov) [#37660](https://github.com/nodejs/node/pull/37660)
* **(SEMVER-MAJOR)** **lib,src**: Parent를 사용하도록 클러스트를 업데이트했습니다. (Michael Dawson) [#36478](https://github.com/nodejs/node/pull/36478)
* **(SEMVER-MINOR)** **module**: `node:`로 시작하는 `require(…)` 호출 지원을 추가했습니다. (ExE Boss) [#37246](https://github.com/nodejs/node/pull/37246)
* **(SEMVER-MINOR)** **perf_hooks**: timerify에 히스토그램 옵션을 추가했습니다. (James M Snell) [#37475](https://github.com/nodejs/node/pull/37475)
* **(SEMVER-MINOR)** **repl**: `node:`로 시작하는 `require(…)`호출에 자동완성을 추가했습니다. (ExE Boss) [#37246](https://github.com/nodejs/node/pull/37246)
* **(SEMVER-MINOR)** **util**: getSystemErrorMap() 구현을 추가했습니다. (eladkeyshawn) [#38101](https://github.com/nodejs/node/pull/38101)
