...
Node.js v16.0.0은 애플 실리콘을 위해 미리 빌드한 바이너리를 제공하는 최초 릴리스가 될 것입니다. 우리는 Intel (`darwin-x64`)와 ARM (`darwin-arm64`) 아키택처에 대해 분리된 tarballs를 제공 할 예정이지만 맥OS 인스톨러(`.pkg`)는 ‘fat’ (multi-architecture) 바이너리로 제공될 예정입니다.
