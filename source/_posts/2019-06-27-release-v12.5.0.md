...
* **build**:
  * 기본적으로 V8 스냅샷을 활성화해서 시작 시간을 줄였습니다. [#28181](https://github.com/nodejs/node/pull/28181)
* **deps**:
...
* **inspector**:
  * 인스팩터의 웹 소켓 URL을 노출하는 방법을 지정하는 `--inspect-publish-uid` 플래그를 추가했습니다. [#27741](https://github.com/nodejs/node/pull/27741)
* **n-api**:
  * napi_define_*의 엑세서가 이제 ECMAScript와 호환됩니다. [#27851](https://github.com/nodejs/node/pull/27851)
* **report**:
  * 리포트 출력에 cpu 정보가 추가되었습니다. [#28188](https://github.com/nodejs/node/pull/28188)
* **src**:
  * stdio를 raw나 논블로킹 모드에서 남겨두지 않도록 종료할 때 stdio 파일 디스크립터를 원래 상태로 복원합니다. [#24260](https://github.com/nodejs/node/pull/24260)
* **tools,gyp**:
...
* **worker**:
  * `worker.terminate()`이 이제 promise를 반환하고 콜백 사용은 폐기 예정입니다.
