---
category: articles
title: Node v12.6.0(현재 버전)
author: Michaël Zasso
ref: Node v12.6.0 (Current)
refurl: https://nodejs.org/en/blog/release/v12.6.0
...
* **build**:
  * MIPS 아키텍처에서의 Node.js 빌드를 실험적으로 다시 지원합니다. [#27992](https://github.com/nodejs/node/pull/27992).
* **child_process**:
  * 프로미스 버전의 `child_process.exec`와 `child_process.execFile`가 이제
    `Promise`를 반환하고 이 `Promise`에는 `child` 속성으로 자식 인스턴스가 붙어있습니다.
    [#28325](https://github.com/nodejs/node/pull/28325).
* **deps**:
  * libuv를 1.30.1로 업데이트했습니다. [#28449](https://github.com/nodejs/node/pull/28449), [#28511](https://github.com/nodejs/node/pull/28511).
    * Haiku 플랫폼을 지원합니다.
    * `UV_THREADPOOL_SIZE`의 최댓값이 128에서 1024로 증가했습니다.
    * `uv_fs_copyfile()`가 이제 소스 파일과 목적 파일이 같을 때도 제대로 동작합니다.
* **process**:
  * 새로운 `process.resourceUsage()` 메서드를 추가했습니다. 이는 현재 프로세스의 CPU 시간같은
    리소스 사용량을 반환합니다. [#28018](https://github.com/nodejs/node/pull/28018).
* **src**:
  * 특정 상황에서 프로세스에 크래시를 발생시킬 수 있는 stdio 관련 이슈를 수정했습니다.
    [#28490](https://github.com/nodejs/node/pull/28490).
* **stream**:
  * 쓰기가 가능한 스트림에 `writableFinished` 프로퍼티를 추가했습니다. 이는 모든 데이터를 기반
    시스템에 다 내보냈는지를 나타냅니다. [#28007](https://github.com/nodejs/node/pull/28007).
* **worker**:
  * 워커 스레드가 stdin에서 데이터를 받지 못하는 문제를 수정했습니다. [#28153](https://github.com/nodejs/node/pull/28153).
* **meta**:
  * [Jiawen Geng](https://github.com/gengjiawen)을 협업자로 추가했습니다. [#28322](https://github.com/nodejs/node/pull/28322).
