...
`diagnostics_channel`은 진단을 위한 임의의 메시지를 보고할 이름있는 채널을 생성하는 API를 제공하는 새 실험적 모듈입니다.

`diagnostics_channel`로 Node.js 코어와 모듈 작성자는 주어진 시간에 무엇을 했는지에 관한 컨텍스트 데이터를
게시할 수 있습니다. 예를 들어 mysql 쿼리의 호스트명과 쿼리스트링이 될 수 있습니다. `dc.channel(name)`로 이름있는
...
채널은 하나의 큰 글로벌 이벤트 이미터와 비슷하지만 최상의 성능을 얻기 위해 별도의 객체로 나뉘어 집니다. 아무도 채널을 수신하고 있지 않다면
게시를 위한 오버헤드는 가능한 한 0에 가까워야 합니다. 채널 데이터를 보시하는 것은 해당 채널에 메시지가 게시될 때마다 함수가 실행되도록
`channel.subscribe(listener)`를 사용하는 것처럼 쉽습니다.
...
전달받은 데이터는 주어진 시간에 앱이 무엇을 하는지에 관한 컨텍스트를 제공하는데 사용할 수 있습니다. 추적 데이터 강화,
...
생성된 자식 프로세스 **내에서** 오류 발생하더라도 상관없이 `'spawn'` 이벤트는 발생합니다. 예시로 `bash some-command`가
...
리졸버는 IPv4 DNS 서버에 요청을 만들 때 v4 로컬 주소를 사용할 것이고 IPv6 DNS 서버에 요청을 생성할 때는 v6 로컬 주소를 사용할 것이다.
...
resolver.setLocalAddress('10.1.2.3');
// resolver.setLocalAddress('10.1.2.3', '::0');와 같다.
```
...
`NODE_V8_COVERAGE` 환경 변수로 시작되는 V8 커버리지를 제어하는 두 개의 새 메서드가 `v8` 모듈은 추가되었습니다.

`v8.takeCoverage()`로 필요할 때 디스크에 커버리지 보고서를 작성할 수 있습니다. 프로세스가 살아있는 동안 여러 번 실행할 수 있고
실행 카운터는 호출 시마다 리셋될 것이다. 프로세스가 종료되려고 할 때 미리 `v8.stopCoverage()`를 호출하지 않는 한
마지막 커버리지가 디스크에 쓰일 것입니다.

`v8.stopCoverage()` 메서드로 커버리지 수집을 멈추게 해서 V8이 실행 카운터를 해제하고 코드를 최적화할 수 있다.
...
새로운 `--heapsnapshot-near-heap-limit=max_count` 실험적 명령행 플래그를 사용해서 V8 힙 사용량이 힙 한도에
근접할 때 힙 스냅숏을 자동으로 생성하게 할 수 있습니다. `count`는 음수가 아닌 정수여야 합니다.
(이 경우 Node.js는 디스크에 `max_count` 스냅숏만큼만 생성할 것입니다.)

스냅숏을 생성할 때 가비지 컬렉션이 실행되고 힙 사용량이 줄어들 수 있음으로 Node.js 인스턴스가 결국 out of memory가
...
힙이 클수록 더 많은 리소스가 필요합니다. 추가적인 V8 힙 메모리의 오버헤드를 조절하려고 V8 힙을 조정하고 프로세스가 사용할 수 있는
