...
Node.js 12.16.0 에는 이번 릴리즈에 수정된 여섯 개의 회귀들이 포함되어 있습니다.

**자기 모듈 처리(Self Resolving Modules)의 잘못된 플래그 제거**:

12.16.0 에는 ESM 구현과 관련하여 대규모 업데이트가 포함되었습니다. 새로운 기능들 중 하나는 자기 지시적 모듈(Self Referential Modules) 인데, `--experimental-modules` 플래그하지 않은 상태로 잘못 릴리스 되었었습니다. 이번 릴리스에서는 이 기능을 적절하게 플래그하고 있습니다.
