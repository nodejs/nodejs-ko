---
category: articles
title: Node v8.0.0(현재 버전)
author: James M Snell, Colin Ihrig
ref: Node v8.0.0 (Current)
refurl: https://nodejs.org/en/blog/release/v8.0.0
translator:
...
[릴리스를 발표](http://blog.npmjs.org/post/161081169345/v500)했고
...
[Node.js 네이티브 추상화(`nan`)](https://www.npmjs.com/package/nan)를 뛰어넘는
큰 향상을 보여줍니다.
...
Node.js 8.0.0에서 [N-API](https://nodejs.org/api/n-api.html)는 실험단계이므로
구현체와 API에서 많은 변경이 있을 것입니다. 네이티브 애드온 개발자에게는 최대한 빨리
[이 API로 작업해보기를 권장](https://nodejs.org/en/docs/guides/publishing-napi-modules/)하고
...
[전체 문서](https://github.com/nodejs/node/pull/13287)는 아직 작업중이고
...
[WHATWG URL 표준](https://url.spec.whatwg.org/)을 구현한 실험적인 `URL` API는
작년 Node.js 7.x에 추가된 후 활발하게 개발되었습니다. 8.0.0에서 새로운 `URL` 구현체가 완전히
지원해서 실험적인 API가 아님을 발표할 수 있게 되었습니다. 사용 예시는 아래에 나와 있고 더 자세한
내용은 [공식 문서](https://nodejs.org/api/url.html#url_the_whatwg_url_api)에서
볼 수 있습니다.
...
이 새로운 `URL` 구현체는 Chrome, Firefox, Edge, Safari 같은 최신 웹 브라우저에서
사용할 수 있는 `URL` 구현체 및 API와 일치하므로 URL을 사용하는 코드를 다양한 환경에서
공유할 수 있다는 점이 중요합니다.
...
Node.js 6.0.0에서 다수의 [보안](https://nodesecurity.io/advisories/67) 문제와
사용성 문제를 해결하려고 `Buffer(num)` 생성자 호출 대신 사용하는 새로운 `Buffer` 생성자 메서드가
[도입되었습니다](https://medium.com/@jasnell/node-js-buffer-api-changes-3c21f1048f97).
...
현재는 `Buffer(num)` 생성자를 Node.js에서 제거할 계획이 전혀 없지만,
이 생성자의 사용은 여전히 폐기예정입니다.
...
개발 중이나 CI 테스트 환경에서 애플리케이션의 `Buffer(num)` 사용을 찾아내기 쉽도록
`--pending-deprecation` 명령행 플래그와 이와 같은 `NODE_PENDING_DEPRECATION=1`
환경 변수가 추가되어 `Buffer(num)`을 사용하면(그 외 잠재적인 지연된 폐기예정도 포함해서)
Node.js가 `DeprecationWarning` 프로세스 경고를 보여줄 것입니다. 이 폐기예정 기능이
프로덕션 애플리케이션에는 영향을 주지 않도록 이 플래그는 기본적으로는 꺼져 있습니다.
아래에 지연된 폐기예정을 활성화하는 예시가 있습니다.
...
Node.js 8.0.0에는 새로운 `util.promisify()` API가 포함되어 표준 Node.js 콜백 스타일의
API를 `Promise`를 반환하는 함수로 감쌀 수 있게 되었습니다.
아래는 `util.promisify()`의 사용 예시입니다.
...
예를 들어 `assert(false)`를 호출하면 다음의 `AssertionError`를 생성할 것입니다.
...
정적 오류 코드에 대한 정보는 Node.js 문서를 참고하면 빠르게 찾아볼 수 있습니다. 예시로
`ERR_ARG_NOT_ITERABLE` 오류 코드에 대한 정보를 검색하는 URL은
<https://nodejs.org/dist/latest-v8.x/docs/api/errors.html#ERR_ARG_NOT_ITERABLE>입니다.
...
기본 동작인 `stderr`에 경고를 출력하는 대신 경고를 지정한 파일에 작성해서 애플리케이션의
...
### 실험적인 인스펙터 자바스크립트 API

Inspector 프로토콜을 사용하는 새로운 실험 자바스크립트 API가 도입되어 개발자가 실행 중인
Node.js 프로세스를 검사하는 디버그 프로토콜을 사용하는 새로운 방법을 이용할 수 있게 되었습니다.
...
// inspector 이벤트를 감시합니다
session.on('inspectorNotification', (message) => { /** ... **/ });

// inspecotr에 메시지를 보냅니다
session.post(message);

session.disconnect();
```

inspector API는 실험 단계이므로 크게 달라질 수 있습니다.
...
* **Async Hooks**
  * `async_hooks` 모듈이 코어에 추가되었습니다.
    [[`4a7233c178`](https://github.com/nodejs/node/commit/4a7233c178)]
    [#12892](https://github.com/nodejs/node/pull/12892).

* **Buffer**
  * `--pending-deprecation` 플래그를 사용하면 `new Buffer(num)`나 `Buffer(num)`를
    사용할 때 Node.js가 폐기예정 경고를 보여줍니다.
    [[`d2d32ea5a2`](https://github.com/nodejs/node/commit/d2d32ea5a2)]
    [#11968](https://github.com/nodejs/node/pull/11968).
  * `new Buffer(num)`와 `Buffer(num)`는 새로운 `Buffer` 인스턴스를 0으로 채웁니다.
    [[`7eb1b4658e`](https://github.com/nodejs/node/commit/7eb1b4658e)]
    [#12141](https://github.com/nodejs/node/pull/12141).
  * 많은 `Buffer` 메서드가 이제 `Uint8Array`를 입력으로 받습니다.
    [[`beca3244e2`](https://github.com/nodejs/node/commit/beca3244e2)]
    [#10236](https://github.com/nodejs/node/pull/10236).
...
* **Child Process**
  * 인자와 kill 신호의 유효성 검사를 개선했습니다.
    ...
    [[`d75fdd96aa`](https://github.com/nodejs/node/commit/d75fdd96aa)]
    [#10423](https://github.com/nodejs/node/pull/10423).
  * Child Process 메서드가 `Uint8Array`를 입력으로 받습니다.
    [[`627ecee9ed`](https://github.com/nodejs/node/commit/627ecee9ed)]
    [#10653](https://github.com/nodejs/node/pull/10653).
...
* **Domains**
  * 네이티브 `Promise` 인스턴스가 이제 `Domain`을 인식합니다.
    [[`84dabe8373`](https://github.com/nodejs/node/commit/84dabe8373)]
    [#12489](https://github.com/nodejs/node/pull/12489).
...
* **File System**
  * 유틸리티 클래스 `fs.SyncWriteStream`이 폐기예정이 되었습니다.
    [[`7a55e34ef4`](https://github.com/nodejs/node/commit/7a55e34ef4)]
    [#10467](https://github.com/nodejs/node/pull/10467).
...
* **HTTP**
  ...
  * `httpResponse.writeHeader()` 메서드가 폐기예정이 되었습니다.
    [[`fb71ba4921`](https://github.com/nodejs/node/commit/fb71ba4921)]
    [#11355](https://github.com/nodejs/node/pull/11355).
  * HTTP 헤더에 접근하는 새로운 메서드를 `OutgoingMessage`에 추가했습니다.
    [[`3e6f1032a4`](https://github.com/nodejs/node/commit/3e6f1032a4)]
    [#10805](https://github.com/nodejs/node/pull/10805).
...
* **Src**
  * `NODE_MODULE_VERSION`을 57로 업데이트했습니다.
    [[`ec7cbaf266`](https://github.com/nodejs/node/commit/ec7cbaf266)]
    [#12995](https://github.com/nodejs/node/pull/12995).
  ...
  * `--debug` 명령행 인자가 폐기예정이 되었습니다. Node.js가 이전에 사용하던 레거시 Debugger
    프로토콜이 제거되었으므로 `--debug`를 사용하면 *새로운* Inspector에 기반을 둔
    디버그 프로토콜을 활성화할 것입니다.
    [[`010f864426`](https://github.com/nodejs/node/commit/010f864426)]
    [#12949](https://github.com/nodejs/node/pull/12949).
  * `-c`와 `-e` 명령행 인자를 동시에 사용하면 예외를 던집니다.
    [[`a5f91ab230`](https://github.com/nodejs/node/commit/a5f91ab230)]
    [#11689](https://github.com/nodejs/node/pull/11689).
  * `--use-bundled-ca`와 `--use-openssl-ca` 명령행 인자를 동시에 사용하면 예외를 던집니다.
    [[`8a7db9d4b5`](https://github.com/nodejs/node/commit/8a7db9d4b5)]
    [#12087](https://github.com/nodejs/node/pull/12087).

* **Stream**
  * `Stream`이 이제 `destroy()`와 `_destroy()` API를 지원합니다.
    [[`b6e1d22fa6`](https://github.com/nodejs/node/commit/b6e1d22fa6)]
    [#12925](https://github.com/nodejs/node/pull/12925).
  * `Stream`이 이제 `_final()` API를 지원합니다.
    [[`07c7f198db`](https://github.com/nodejs/node/commit/07c7f198db)]
    [#12828](https://github.com/nodejs/node/pull/12828).

* **TLS**
  * `rejectUnauthorized` 옵션의 기본이 이제 `true`가 되었습니다.
    [[`348cc80a3c`](https://github.com/nodejs/node/commit/348cc80a3c)]
    [#5923](https://github.com/nodejs/node/pull/5923).
  * `tls.createSecurePair()` API는 이제 런타임 폐기예정을 발생시킵니다.
    [[`a2ae08999b`](https://github.com/nodejs/node/commit/a2ae08999b)]
    [#11349](https://github.com/nodejs/node/pull/11349).
  * `dhparam`이 2048비트보다 적으면 런타임 폐기예정이 발생할 것입니다.
    [[`d523eb9c40`](https://github.com/nodejs/node/commit/d523eb9c40)]
    [#11447](https://github.com/nodejs/node/pull/11447).
...
* **Util**
  * `util.inspect()`를 사용할 때 `Symbol` 키가 기본적으로 보입니다.
    [[`5bfd13b81e`](https://github.com/nodejs/node/commit/5bfd13b81e)]
    [#9726](https://github.com/nodejs/node/pull/9726).
  * `%j`를 포매팅할 때 `toJSON` 오류를 던질 것입니다.
    [[`455e6f1dd8`](https://github.com/nodejs/node/commit/455e6f1dd8)]
    [#11708](https://github.com/nodejs/node/pull/11708).
  * `inspect.styles`와 `inspect.colors`를 prototype이 없는 객체로 변경했습니다.
    [[`aab0d202f8`](https://github.com/nodejs/node/commit/aab0d202f8)]
    [#11624](https://github.com/nodejs/node/pull/11624).
...
* **Zlib**
  * Zlib에서 편의 메서드 `Uint8Array`를 지원합니다.
    [[`91383e47fd`](https://github.com/nodejs/node/commit/91383e47fd)]
    [#12001](https://github.com/nodejs/node/pull/12001).
  * Zlib 오류가 이제 시종일관 `RangeError`와 `TypeError`를 사용합니다.
    [[`b514bd231e`](https://github.com/nodejs/node/commit/b514bd231e)]
    [#11391](https://github.com/nodejs/node/pull/11391).
