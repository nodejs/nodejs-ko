...
* **fs**:
  ...
  * `ReadStream` 또는 `WriteSream`에서 `open()` 메서드를 호출하면 이제 런타임 폐기 경고가 발생합니다. 메서드는 내부에 있어야하며 사용자의 코드로 호출하면 안됩니다. (Robert Nagy) [#29061](https://github.com/nodejs/node/pull/29061).
  * `fs.read/write`, `fs.readSync/writeSync` 및 `fd.read/write` 는 이제 `offset` 파라미터로 안전한 정수를 받아들입니다. `offset`의 값도 더 이상 강제되지 않으므로 유효한 타입이 함수에 전달되어야 합니다. (Zach Bjornson) [#26572](https://github.com/nodejs/node/pull/26572).
* **http**:
  * 중단 된 요청은 더 이상 `aborted` 된 후에 `end` 또는 `error` 이벤트를 생성하지 않습니다.
    (Robert Nagy) [#27984](https://github.com/nodejs/node/pull/27984), [#20077](https://github.com/nodejs/node/pull/20077).
  * 소켓 오류 발생 이후 데이터가 더 이상 방출되지 않습니다. (Robert Nagy) [#28711](https://github.com/nodejs/node/pull/28711).
