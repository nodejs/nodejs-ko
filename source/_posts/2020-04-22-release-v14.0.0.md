---
category: articles
title: Node v14.0.0(현재 버전)
author: Bethany Nicolle Griggs
ref: Node v14.0.0 (Current)
refurl: https://nodejs.org/en/blog/release/v14.0.0
...
* **(SEMVER-MAJOR)** **crypto**: 다이제스트가 없는 pbkdf2를 EOL(End-of-Life)로 바꾸었습니다. (James M Snell) [#31166](https://github.com/nodejs/node/pull/31166)
* **(SEMVER-MAJOR)** **fs**: 가비지 컬렉션 중에 FileHandle을 닫는 동작이 폐기 예정이 되었습니다. (James M Snell) [#28396](https://github.com/nodejs/node/pull/28396)
* **(SEMVER-MAJOR)** **http**: OutboundMessage.prototype.flush를 EOL로 바뀌었습니다. (James M Snell) [#31164](https://github.com/nodejs/node/pull/31164)
* **(SEMVER-MAJOR)** **lib**: GLOBAL과 root 별칭을 EOL로 바뀌었습니다. (James M Snell) [#31167](https://github.com/nodejs/node/pull/31167)
* **(SEMVER-MAJOR)** **os**: tmpDir()를 EOL로 바꾸었습니다. (James M Snell) [#31169](https://github.com/nodejs/node/pull/31169)
* **(SEMVER-MAJOR)** **src**: 폐기 예정이었던 wasm 타입 검사를 제거했습니다. (Clemens Backes) [#32116](https://github.com/nodejs/node/pull/32116)
* **(SEMVER-MAJOR)** **stream**: \_writableState.buffer를 EOL로 바꾸었습니다. (James M Snell) [#31165](https://github.com/nodejs/node/pull/31165)
* **(SEMVER-MINOR)** **doc**: process.mainModule이 폐기 예정이 되었습니다. (Antoine du HAMEL) [#32232](https://github.com/nodejs/node/pull/32232)
* **(SEMVER-MINOR)** **doc**: 인자가 없는 process.umask()는 폐기 예정이 되었습니다. (Colin Ihrig) [#32499](https://github.com/nodejs/node/pull/32499)
...
* **module**: 실험적인 모듈 경고를 제거했습니다. (Guy Bedford) [#31974](https://github.com/nodejs/node/pull/31974)
...
Node.js 14부터는 Node.js에서 ESM을 사용할 때 이 경고가 없습니다. 하지만 Node.js의 ESM 구현체를 아직 실험적입니다. 안정성 지수에 따르면 "이 기능은 유의적 버전(Semantic Versioning) 규칙의 대상이 아닙니다. 차후 릴리스에서 하위 호환성을 깨뜨리는 변경이나 제거가 있을 수 있습니다." 프로덕션 환경에서 이 기능을 사용할 때 사용자는 주의해야 합니다.
...
명세 준수와 웹 호환성이라는 두 가지 주요 목표에 따라 많은 디자인 결정이 이루어졌음을 강조할 필요가 있습니다. 현재의 구현체가 미래의 증거 모델
유니버설 자바스크립트로 가는 길의 기반을 닦아주는 ESM 모듈 작성의 미래 증거 모델이 될 것이라고 믿고 있습니다. 자세한 내용을 문서를 참고하세요.
...
* **src**: 새로운 V8 ArrayBuffer API로 마이그레이션했습니다. (Thang Tran) [#30782](https://github.com/nodejs/node/pull/30782)

V8이 같은 기반 주소를 가리키는 여러 ArrayBuffers를 더는 허용하지 않습니다. 이는 네이티브 애드온에 영향을 줄 것입니다.
...
* **(SEMVER-MAJOR)** **build**: 14.x에서 macos 배포 대상을 10.13으로 업데이트했습니다. (AshCripps) [#32454](https://github.com/nodejs/node/pull/32454)
* **(SEMVER-MAJOR)** **doc**: Linux armv7를 위한 크로스 컴파일러 머신을 업데이트했습니다. (Richard Lau) [#32812](https://github.com/nodejs/node/pull/32812)
* **(SEMVER-MAJOR)** **doc**: devtoolset-8을 사용해서 Centos/RHEL 릴리스를 업데이트했습니다. (Richard Lau) [#32812](https://github.com/nodejs/node/pull/32812)
* **(SEMVER-MAJOR)** **doc**: 공식 바이너리에서 SmartOS를 제거했습니다. (Richard Lau) [#32812](https://github.com/nodejs/node/pull/32812)
* **(SEMVER-MAJOR)** **win**: 지원이 끝난 Windows 버전에서 실행되는 것을 막았습니다. (João Reis) [#31954](https://github.com/nodejs/node/pull/31954)

ARM에서 Node.js 바이너리와 네이티브 애드온 간에 ABI가 맞지 않을 것입니다. `std::shared_ptr`와 상호작용하는 네이티브 애드온만 깨질 것입니다. Node.js 14의 차후 버전에서 수정할 수 있을 것으로 기대하고 있습니다. - [#30786](https://github.com/nodejs/node/issues/30786)
...
* **(SEMVER-MAJOR)** **deps**: V8을 8.1.307.20으로 업데이트했습니다. (Matheus Marchini) [#32116](https://github.com/nodejs/node/pull/32116)
  * 기본적으로 옵셔널 체이닝이 활성화되었습니다. ([MDN](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Operators/Optional_chaining), [v8.dev](https://v8.dev/features/optional-chaining))
  * 기본적으로 널 병합 연산자가 활성화되었습니다. ([MDN](https://wiki.developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Operators/Nullish_Coalescing_Operator), [v8.dev](https://v8.dev/features/nullish-coalescing))
  * 기본적으로 `Intl.DisplayNames`가 활성화되었습니다. ([MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DisplayNames), [v8.dev](https://v8.dev/features/intl-displaynames))
  * 기본적으로 `Intl.DateTimeFormat`의 `calendar`와 `numberingSystem` 옵션이 활성화되었습니다. ([MDN](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Global_Objects/DateTimeFormat))
...
* **cli, report**: --report-on-fatalerror이 안정(stable)으로 바뀌었습니다. (Colin Ihrig) [#32496](https://github.com/nodejs/node/pull/32496)
* **deps**: libuv를 1.37.0으로 업그레이드했습니다. (Colin Ihrig) [#32866](https://github.com/nodejs/node/pull/32866)
* **fs**: fs/promises 별칭 모듈을 추가했습니다. (Gus Caplan) [#31553](https://github.com/nodejs/node/pull/31553)
