...
author: Myles Borins
ref: Node v4.5.0 (LTS)
refurl: https://nodejs.org/en/blog/release/v4.5.0/
translator:
---
...
* **buffer**:
 * 새 버퍼 생성자 API를 v4.x에 백포트했습니다.(Сковорода Никита Андреевич) [#7562](https://github.com/nodejs/node/pull/7562)
 * backport cli 옵션 --zero-fill-buffers를 백포트했습니다.(James M Snell) [#5745](https://github.com/nodejs/node/pull/5745)
* **build**:
  * Intel Vtune 프로파일링을 지원합니다.(Chunyang Dai) [#5527](https://github.com/nodejs/node/pull/5527)
* **repl**:
  * 탭의 복사로 자동완성이 일어나지 않습니다.(Eugene Obrezkov) [#5958](https://github.com/nodejs/node/pull/5958)
* **src**:
  * node::FreeEnvironment 공개 API를 추가했습니다.(Cheng Zhao) [#3098](https://github.com/nodejs/node/pull/3098)
* **test**:
  * 노드 트리에서 v8 테스트를 실행합니다.(Bryon Leung) [#4704](https://github.com/nodejs/node/pull/4704)
* **V8**:
  * 객체의 조사와 함수의 문맥 값 조사를 향상하기 위해 post mortem 데이터를 추가했습니다.(Fedor Indutny) [#3779](https://github.com/nodejs/node/pull/3779)

Semver Patch:

* **buffer**:
  * 음수인 할당 길이를 무시합니다.(Anna Henningsen) [#7562](https://github.com/nodejs/node/pull/7562)
* **crypto**:
  * 루트 인증서를 갱신했습니다.(Ben Noordhuis) [#7363](https://github.com/nodejs/node/pull/7363)
* **libuv**:
  * libuv를 1.9.1 버전으로 업그레이드 했습니다.(Saúl Ibarra Corretgé) [#6796](https://github.com/nodejs/node/pull/6796)
  * libuv를 1.9.0 버전으로 업그레이드 했습니다.(Saúl Ibarra Corretgé) [#5994](https://github.com/nodejs/node/pull/5994)
* **npm**:
  * 2.15.9 버전으로 업그레이드 했습니다.(Kat Marchán) [#7692](https://github.com/nodejs/node/pull/7692)
