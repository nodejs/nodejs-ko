---
category: articles
title: Node v9.6.1(현재 버전)
author: Myles Borins
ref: Node v9.6.1 (Current)
refurl: https://nodejs.org/en/blog/release/v9.6.1
...
이번 릴리스는 이미 배포된 v9.6.0의 잠재적인 Semver-Major 회기를 수정한 특별한 릴리스입니다.

## 주요 변경사항

* **events**:
  - 9.5.0에서 변경된 `events.usingDomains`는 기본 값이 `false`였고 이는 9.6.0에서 제거되었습니다. 이 변화는 되돌려 졌고, 9.6.0이전의 9.x에서 사용되던 `events` 객체는 이제 `usingDomains`을 미리 `false`로 할당하고 있습니다. (Myles Borins) [#18944](https://github.com/nodejs/node/pull/18944)
