---
category: articles
title: Node v7.3.0(현재 버전)
author: Colin Ihrig
ref: Node v7.3.0 (Current)
refurl: https://nodejs.org/en/blog/release/v7.3.0
...
* **buffer**:
  - buffer.fill()이 빅엔디안(Big-Endian) 환경의 UCS2 인코딩에서도 정상적으로 동작합니다. (Anna Henningsen) [#9837](https://github.com/nodejs/node/pull/9837)
* **cluster**:
  - disconnect()가 접속이 종료된 워커의 참조를 반환합니다. (Sean Villars) [#10019](https://github.com/nodejs/node/pull/10019)
* **crypto**:
  - NODE_EXTRA_CA_CERTS 환경 변수를 사용해 내장된 잘 알려진 인증 기관(Certificate Authorities) 목록을 확장할 수 있습니다. (Sam Roberts) [#9139](https://github.com/nodejs/node/pull/9139)
* **http**:
  - Keep Alive를 사용할 때 메모리 누수를 방지하기 위해 만료된 타임아웃 리스너를 제거합니다. (Karl Böhlmark) [#9440](https://github.com/nodejs/node/pull/9440)
* **tls**:
  - 명시적인 키/암호문(key/passphrase) 조합을 사용할 수 있습니다. (Sam Roberts) [#10294](https://github.com/nodejs/node/pull/10294)
* **url**:
  - 명세 사항을 충족하기 위해 URL.originFor()에 기본 위치(base) 인수를 추가했습니다. (joyeecheung) [#10021](https://github.com/nodejs/node/pull/10021)
  - 명세 사항을 충족하기 위해 URLSearchParams을 개선했습니다. (Timothy Gu) [#9484](https://github.com/nodejs/node/pull/9484)
