...
Node.js 12.17.0에서 ESM 모듈을 사용할 때 `--experimental-modules` 플래그를 사용할 필요가 없어졌지만, Node.js의 ESM 구현체는 아직 실험적입니다. 안정성 지수에 따르면 "이 기능은 유의적 버전(Semantic Versioning) 규칙의 대상이 아닙니다. "이 기능은 유의적 버전(Semantic Versioning) 규칙의 대상이 아닙니다. 차후 릴리스에서 하위 호환성을 깨뜨리는 변경이나 제거가 있을 수 있습니다." 사용자는 프로덕션 환경에서 이 기능을 사용할 때 주의해야 합니다.

Node.js 14와는 달리, ESM을 사용하거나 모듈이 애플리케이션의 시작점(entrypoint) 혹은 동적인 `import()`가 맨 처음 호출되는 시점에 런타임 실험상태 경고가 발생할 것입니다.

Node.js의 ESM 구현체는 개발자가 익숙한 경험과는 다릅니다. 대부분의 트랜스파일 워크플로우가 지원하는 것은 CommonJS 모듈의 import로부터 명명된 export들, 선택적인 파일 확장, 또는 Node.js ESM 구현체가 지원하지 않는 JSON 모듈과 같은 기능들입니다. 트랜스파일된 환경의 모듈을 Node.js에서 동작하도록 어느 정도 리팩토링해야 할 가능성이 높습니다. 명세 준수와 웹 호환성이라는 두 가지 주요 목표에 따라 많은 디자인 결정이 이루어졌음을 강조할 필요가 있습니다. 현재의 구현체가
...
`AsyncLocalStorage` 클래스가 Async Hooks 모듈에 소개되었습니다.

이 API는 컨텍스가 비동기 운영될 수 있도록 합니다. 예를 들어, 만약 서버로 HTTP 요청을 할 때 `AsyncLocalStorage` 인스턴스에 시퀀스 id가 저장되면, 현재 HTTP 요청에 접근할 필요없이 이 id를 가져올 수 있습니다.
...
이 예시 코드에서, `logWithId` 함수는 현재 요청 id 가 무엇인지, 언제 다수의 요청이 동시에 발생하였는지 알려줍니다.
...
*Note: 이 API는 아직 실험적이며 몇몇 메서드들은 향후 Node.js 릴리스에서 변경될 수 있습니다.*
...
역점진탐색과 관계없는 아무 버튼을 누르자마자 바로 엔트리에 접근합니다. 취소는 `escape` 또는 `<ctrl> + C`를 누르면 됩니다.
...
콘솔 생성자(`require('console').Console`)가 이제 그룹 인덴트를 지원합니다.
...
rickyes가 기여하였습니다. - [#32964](https://github.com/nodejs/node/pull/32964).

#### `util.inspect()`를 위한 `maxStringLength` 옵션

오브젝트들을 검사하는 동안 문자열의 길이를 정할 수 있습니다. 아래와 같이 `maxStringLength` 옵션을 전달하면 됩니다.:
...
#### 안정적인 진단 보고 Stable diagnostic reports

[Diagnostic Report](https://nodejs.org/dist/latest-v12.x/docs/api/report.html)는 이제 안정적입니다. 새로운 `--report-compact` 플래그를 지원하기 때문에 간결한 한 줄 짜리의 JSON 형식으로 보고서를 작성할 수 있고, 로그 처리 시스템은 기본 멀티라인 형식보다 사람이 사용하기에 더 용이하게 디자인되었습니다.

#### 서버 헤더의 기본 만료시간 연장

`http`와 `https`의 `server.headersTimeout` 기본값을 `40000`에서 `60000`(60초)으로 늘렸습니다. AWS ELB와 같이 60초 만료시간을 가지고 있는 시스템 정책과 맞추었습니다.

Tim Costa가 기여하였습니다. - [#30071](https://github.com/nodejs/node/pull/30071).
...
* **crypto**:
  * Diffie-Hellman 방식을 지원합니다. (Tobias Nießen) [#31178](https://github.com/nodejs/node/pull/31178).
* **dns**:
