...
검색하고 싶은 과거 입력 코드의 맨 앞 문자들을 입력하여 히스토리 엔트리에 빠르게 접근할 수 있습니다. 그 문자들로 시작하는 히스토리 엔트리를 훑어보려면 `<UP>` 또는 `<DOWN>`을 누르면 됩니다.
...
`EventEmitter.errorMonitor` 심볼을 사용해서 리스너를 설치하면 발생한 에러를 소비하지 않고 `EventEmitter`로 `'error'` 이벤트를 모니터링할 수 있습니다.
...
#### uncaught exception 모니터링

`'uncaughtExceptionMonitor'` 리스너를 설치하면 프로세스를 종료하는 기본 동작을 오버라이딩하지 않아도 `'uncaughtException'` 이벤트를 모니터링할 수 있습니다.
...
(sync와 프로미스 버전을 포함한) 새로운 함수는 `ArrayBufferView` 요소로 구성된 배열을 받아, 읽은 데이터를 순차적으로 이 버퍼에 기록합니다.

Sk Sajidul Kadir가 기여하였습니다. - [#32356](https://github.com/nodejs/node/pull/32356).

##### `fs.read`의 선택적 파라미터

`fs.read`의 새로운 오버로드(sync와 프로미스 버전 포함)가 사용 가능합니다. `offset`, `length`, `position` 중에서 어떤 것으로든 선택적으로 전달할 수 있습니다.
