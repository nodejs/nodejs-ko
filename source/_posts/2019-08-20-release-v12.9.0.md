...
* **stream**:
  * 읽을 수 있는 스트림에 `readableEnded` 속성을 새로 추가했습니다. `'end'` 이벤트가 발생했을 때 이 값이 `true`로 설정됩니다. (Robert Nagy) [#28814](https://github.com/nodejs/node/pull/28814).
  * 쓸 수 있는 스트림에 `writableEnded` 속성을 새로 추가했습니다. `writable.end()`가 호출된 후 이 값이 `true`로 설정됩니다. (Robert Nagy) [#28934](https://github.com/nodejs/node/pull/28934).
