---
category: articles
title: Node v5.1.0(안정 버전)
author: Jeremiah Senkpiel
ref: Node v5.1.0 (Stable)
refurl: http://nodejs.org/en/blog/release/v5.1.0/
translator:
...
* **buffer**: `noAssert` 옵션이 켜진 상태에서 버퍼 함수를 실행할 때 잘못된 쓰기 값을 만나면 강제 종료되는 대신 잘못된 값을 조용히 무시합니다. (P.S.V.R) [#3767](https://github.com/nodejs/node/pull/3767)
  - 이제 문서에 서술된 대로 동작합니다.
* **child_process**: `child.send()`가 문서에 기술된 것처럼 불리언 값을 적절하게 반환합니다. (Rich Trott) [#3577](https://github.com/nodejs/node/pull/3577)
...
* **repl**: 이제 [이력 저장(persistent history)](https://nodejs.org/api/repl.html#repl_persistent_history) 파일을 열 수 없어도 REPL이 강제 종료되지 않습니다. (Evan Lucas) [#3630](https://github.com/nodejs/node/pull/3630)
* **tls**: `sessionIdContext`의 기본값이 FIPS 모드에서는 MD5 대신 SHA1을 사용하도록 변경됐습니다. (Stefan Budeanu) [#3755](https://github.com/nodejs/node/pull/3755)
* **v8**: 몇 가지 유용한 검사용 데이터를 추가했습니다. (Fedor Indutny) [#3779](https://github.com/nodejs/node/pull/3779)
...
* DNS 질의 진행 중에 호출된 `dns.setServers()`가 단언문 실패로 프로세스를 크래시시킬 수 있습니다. [#894](https://github.com/nodejs/node/issues/894)
