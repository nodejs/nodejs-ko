...
이 메서드들은 첫번째 인자가 문자열인지, 그리고 제공된 정규 표현식과 같은지(혹은 아닌지)를
검증할 것입니다.
...
`child_process`와 `cluster` 모듈은 IPC에 사용되는 직렬화 메커니즘을 변경하기 위해 `serialization` 옵션을 제공합니다. 해당 옵션은 아래 두 가지 값 중 하나를 가지게 됩니다.
...
자발적으로 Node.js 환경을 벗어날 때마다 (예를 들어, `process.exit()` 함수를 실행하거나 Ctrl+C 를 눌러서) Node.js 는 `--trace-exit` 라는 새로운 CLI 플래그는 Node.js 스택 트레이스를 출력합니다.

legendecas [#30516](https://github.com/nodejs/node/pull/30516).

___

새로운 `--trace-uncaught` CLI 플래그는 `Error` 객체가 생성되는 시점이 아니라, 처리되지 않은 예외가 발생하는 시점에 Node.js 스택 트레이스를 출력합니다. 이 옵션은 가비지 컬렉션 동작에 부정적인 영향을 끼칠 수 있기 때문에 기본값으로 사용하지 않습니다. 
...
DSA(전자 서명 알고리즘)과 ECDSA(타원 곡선 전자 서명 알고리즘)을 위한 새로운 서명 인코딩
`verify`와 `sign` 메서드는 `dsaEncoding` 옵션을 받아들이며, 해당 옵션은 아래 두 가지 값 중 하나를 가지게 됩니다. 
For DSA and ECDSA, a new signature encoding is now supported in addition to the
existing one (DER). The `verify` and `sign` methods accept a `dsaEncoding`
option, which can have one of two values:
...
`EventEmitter` 생성자 또는 전역 설정 내부의 실험적인 `captureRejections` 옵션은 `Promise` 에 `.then(undefined, handler)` 핸들러를 설치하여 이 행동을 바꿉니다. 이 핸들러는 만약 예외가 하나 있으면 `Symbol.for('nodejs.rejection')`로 비동기적으로 예외를 연결시킵니다. 만약 예외가 하나도 발생하지 않으면 `'error'` 이벤트 핸들러로 연결합니다. 
...
새로운 핵심 모듈인 `wasi`는 [WebAssembly System Interface](https://wasi.dev/) 설명서를 제공하기 위해 소개됩니다. WASI 는 샌드박스가 적용된 WebAssembly 어플리케이션 접근 
A new core module, `wasi`, is introduced to provide an implementation of the
[WebAssembly System Interface](https://wasi.dev/) specification.
WASI gives sandboxed WebAssembly applications access to the
underlying operating system via a collection of POSIX-like functions.
