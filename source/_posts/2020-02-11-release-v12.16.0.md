...
`Hash`에 새 메서드 `Hash.prototype.copy`가 추가되었습니다. `Hash` 객체의 내부 상태를 복제한 새로운 `Hash` 객체를 만들 수 있어, 여러 업데이트 사이의 다이제스트(digest)를 계산할 수 있게 됩니다.
...
V8이 7.8.279.23으로 업데이트 되었습니다. 객체 디스트럭처링(destructuring), 정규표현식(RegExp) 매치 실패, WebAssembly 가동 시간을 향상시켰습니다.
...
새로운 정적 메서드인 `EventEmitter.on`을 사용하면 이벤트를 비동기적으로 순회할 수 있습니다.
...
`EventEmitter`에서 `EventEmitter.errorMonitor` 심볼로 리스너를 설치하여 `'error'` 이벤트를 확인할 수 있습니다. 이 리스너는 발생한 에러를 소모하지 않습니다.
...
`EventEmitter` 생성자 또는 전역 설정 내부의 실험적인 `captureRejections` 옵션은 `Promise`에 `.then(undefined, handler)` 핸들러를 설치하여 이 행동을 바꿉니다. 이 핸들러는 만약 `Symbol.for('nodejs.rejection')`이 있다면, 비동기적으로 해당 심볼에 예외를 보냅니다. 만약 없다면 `'error'` 이벤트 핸들러로 보냅니다.
...
`EventEmitter.captureRejections = true`로 설정하면 `EventEmitter`의 모든 새로운 인스턴스 기본값이 변경될 것입니다.
