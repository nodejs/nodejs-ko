...
ref: Node v12.16.0 (LTS)
refurl: https://nodejs.org/en/blog/release/v12.16.0
translator: mjayjang
---
...
이제 `assert` 모듈에서는 실험적인 `assert.match()` 와 `assert.doesNotMatch()` 메서드를 제공합니다.
<<<<<<< HEAD
이 메서드들은 첫번째 인자가 문자열인지, 그리고 제공된 정규 표현식과 같은지(혹은 아닌지)를 검증할 것입니다.
=======
이 메서드들은 첫번째 인자가 문자열인지, 그리고 제공된 정규 표현식에 맞는지(혹은 아닌지)를
검증할 것입니다.
>>>>>>> 1899c3190bad9742f2ecdeb08dcf209ac428867e
...
`child_process`와 `cluster` 모듈에서는 IPC에 사용되는 직렬화 메커니즘을 변경하기 위해 `serialization` 옵션을 제공합니다. 해당 옵션은 아래 두 가지 값 중 하나를 가지게 됩니다.

* `'json'` (기본): `JSON.stringify()`와 `JSON.parse()`가 사용됩니다. 이것은 이전에 메시지 직렬화가 작동하던 방식입니다. 
* `'advanced'`: `v8`의 직렬화 API가 사용됩니다. 이것은 [HTML structured clone algorithm](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Structured_clone_algorithm)에 기반하며, `BigInt`, `Map`, `Set` 뿐만 아니라 순환 자료 구조와 같은 더 많은 JavaScript 내장 객체 타입들을 직렬화할 수 있습니다.
...
<<<<<<< HEAD
Node.js 환경을 벗어날 때마다 (예를 들어, `process.exit()` 함수를 실행하거나 Ctrl+C 를 눌러서) Node.js 스택 트레이스는 새로운 CLI 플래그 `--trace-exit`를 출력합니다.
=======
새로운 `--trace-exit` CLI 플래그는 자발적으로 Node.js 환경을 벗어날 때 (예: `process.exit()` 함수 실행, Ctrl+C 입력) Node.js 스택 트레이스를 출력합니다.
>>>>>>> 1899c3190bad9742f2ecdeb08dcf209ac428867e
...
새로운 `--trace-uncaught` CLI 플래그는 `Error` 객체가 생성되는 시점이 아니라, 처리되지 않은 예외가 발생하는 시점에 출력됩니다. 이 옵션은 가비지 컬렉션 동작에 부정적인 영향을 끼칠 수 있기 때문에 기본값으로 사용하지 않습니다.

Anna Henningsen [#30025](https://github.com/nodejs/node/pull/30025).

___

이제 `--disallow-code-generation-from-strings` V8 CLI 플래그는 `NODE_OPTIONS` 환경 변수에 포함됩니다.

Shelley Vohr [#30094](https://github.com/nodejs/node/pull/30094).

#### 새로운 암호화 APIs

DSA(전자 서명 알고리즘)과 ECDSA(타원 곡선 전자 서명 알고리즘)에 사용되는 현재의 서명 인코딩 방식(DER)과 더불어, 새로운 서명 인코딩 방식이 지원됩니다. `verify`와 `sign` 메서드에는 `dsaEncoding` 옵션이 있으며, 해당 옵션은 아래 두 가지 값 중 하나를 가지게 됩니다.

* `'der'` (기본): DER 방식으로 인코딩된 ASN.1 서명 구조 인코딩 `(r, s)`.
* `'ieee-p1363'`: IEEE-P1363 에서 제안한 서명 포맷 `r || s`.

Tobias Nießen [#29292](https://github.com/nodejs/node/pull/29292).

___

`Hash`: `Hash.prototype.copy`에 새로운 방식이 추가되었습니다. 업데이트된 버전들 사이의 다이제스트(digest)를 처리하여 내부 상태의 `Hash` 객체를 새로운 `Hash` 객체로 복제할 수 있습니다.
...
libuv가 1.34.0로 업데이트 되었습니다. 이번 업데이트를 통해 `uv_fs_copyfile()`와 `uv_interface_addresses()` 를 수정하였고 `uv_sleep()`와 `uv_fs_mkstemp()` 새로운 함수 두 개를 추가하였습니다.
...
V8 가 7.8.279.23로 업데이트 되었습니다. 객체 디스트럭처링(destructuring), 정규표현식(RegExp) 맞춤 실패, WebAssembly 가동 시간을 향상시켰습니다.
공식 릴리스 노트는 여기에서 확인 가능합니다. https://v8.dev/blog/v8-release-78.

Michaël Zasso [#30109](https://github.com/nodejs/node/pull/30109).

#### 새로운 EventEmitter API

새로운 정적 메서드인 `EventEmitter.on`를 사용하면 이벤트를 매번 비동기적으로 반복할 수 있습니다.
...
기존에는 `EventEmitter.errorMonitor` 심볼로 리스너를 설치하여 발생한 에러를 확인하였지만, 이제는 `EventEmitter`에서 `'error'` 이벤트들을 확인할 수 있습니다.  
...
이벤트 핸들러와 함께 `async` 함수를 사용하면 문제가 많습니다.
던져진 예외가 있는 경우 처리하지 않은 거부(rejection)를 발생시킬 수 있기 때문입니다.  
...
`EventEmitter` 생성자 또는 전역 설정 내부의 실험적인 `captureRejections` 옵션은 `Promise` 에 `.then(undefined, handler)` 핸들러를 설치하여 이 행동을 바꿉니다. 이 핸들러는 만약 예외가 하나 발생할 경우, 비동기적으로 그것을 `Symbol.for('nodejs.rejection')`로 보냅니다. 만약 예외가 하나도 발생하지 않으면 `'error'` 이벤트 핸들러로 보냅니다. 
...
`EventEmitter.captureRejections = true`로 설정하면 `EventEmitter`의 새로운 인스턴트들을
모두 기본값으로 변경할 것입니다.
...
#### Performance Hook은 더이상 실험적이지 않습니다.

이제 `perf_hooks` 모듈은 안정화되었습니다.

legendecas [#31101](https://github.com/nodejs/node/pull/31101).

#### 실험적인 WASI(WebAssembly System Interface) 지원 안내

새로운 핵심 모듈인 `wasi`는 [WebAssembly System Interface](https://wasi.dev/) 명세의 구현체를 제공하기 위해 등장하였습니다. WASI는 샌드박스가 적용된 WebAssembly 어플리케이션들이 POSIX 같은 함수들의 집합체를 통해 사용중인 운영체제에 접근하는 것을 허용합니다.
