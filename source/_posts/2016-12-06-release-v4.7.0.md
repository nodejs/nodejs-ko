...
author: Myles Borins
ref: Node v4.7.0 (LTS)
refurl: https://nodejs.org/en/blog/release/v4.7.0
translator:
...
* **build**: 내장된 openssl 버전에 링크된 애드온을 빌드할 수 있도록 윈도우에서 openssl 심볼을 외부에 노출합니다. (Alex Hultman) [#7576](https://github.com/nodejs/node/pull/7576)
* **debugger**: 디버거 서버의 listen 주소를 설정할 수 있게 되었습니다. (Ben Noordhuis) [#3316](https://github.com/nodejs/node/pull/3316)
* **dgram**: listening 이벤트 핸들러에서 dgram 소켓이 닫혔을 때 발생할 수 있는 예외를 처리해야 하므로 close를 다룰 수 있도록 send 큐를 일반화했습니다. (Matteo Collina) [#7066](https://github.com/nodejs/node/pull/7066)
* **http**: 451 상태 코드 "Unavailable For Legal Reasons"를 도입했습니다. (Max Barinov) [#4377](https://github.com/nodejs/node/pull/4377)
* **tls**: 클라이언트 인증서, 키, CA 인증서를 캐싱하는 데 유용한 `tls.connect`의 `secureContext`를 도입했습니다. (Fedor Indutny) [#4246](https://github.com/nodejs/node/pull/4246)

주요 SEMVER-PATCH 변경사항은 다음과 같습니다.

* **build**:
  - 임베디드로 사용하는 사용자를 위해 --shared 설정 옵션을 도입했습니다. (sxa555) [#6994](https://github.com/nodejs/node/pull/6994)
* **gtest**: 테스트 보고서가 이제 yaml처럼 tap 주석을 출력합니다. (Johan Bergström) [#9262](https://github.com/nodejs/node/pull/9262)
* **src**: c-ares 초기화를 실패했을 때 Node가 더는 중단되지 않습니다. (Ben Noordhuis) [#8710](https://github.com/nodejs/node/pull/8710)
* **tls**: 핸드쉐이크 과정에서 TLSWrap 인스턴스에 데이터를 쓸 때 발생하는 메모리 누수를 고쳤습니다. (Fedor Indutny) [#9586](https://github.com/nodejs/node/pull/9586)
