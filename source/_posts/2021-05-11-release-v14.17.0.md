...
ref: Node v14.17.0 (LTS)
refurl: https://nodejs.org/en/blog/release/v14.17.0
...
`diagnostics_channel`은 새 실험적 모듈로서, 이름 있는 채널을 생성하는 API를 제공하고 이 채널로 진단을 위한 임의의 메시지를 보고합니다.

이 모듈은 Node.js v15.1.0에서 최초 소개되었으며 더 큰 범위에서 테스트할 수 있도록 v14.17.0에서도 지원합니다.

`diagnostics_channel`로 Node.js 코어와 모듈 작성자는 주어진 시간에 무엇을 했는지에 관한 컨텍스트 데이터를 게시할 수 있습니다. 예를 들어 mysql 쿼리의 호스트명과 쿼리스트링이 될 수 있습니다. `dc.channel(name)`로 이름 있는 채널을 생성하고 `channel.publish(data)`를 호출해서 해당 채널의 모든 리스너에게 데이터를 보내세요.
...
채널은 하나의 큰 글로벌 이벤트 이미터와 비슷하지만 최상의 성능을 얻기 위해 별도의 객체로 나뉩니다. 아무도 채널을 수신하고 있지 않다면 게시를 위한 오버헤드는 가능한 한 0에 가까워야 합니다. channel.subscribe(listener)`를 사용하면 해당 채널에 메시지가 게시될 때마다 함수가 실행되어 채널 데이터를 손쉽게 소비할 수 있습니다.
...
전달받은 데이터는 주어진 시간에 앱이 무엇을 하는지에 관한 컨텍스트를 제공하는 데 사용할 수 있습니다. 추적 데이터 강화, 네트워크나 파일 시스템 활동 추적, 쿼리 로깅 등에 사용될 수 있습니다. 앱이 제공하는 데이터에서 해당 지점의 애플리케이션이 정확히 무엇을 하는지 명확한 그림을 제공하는 진단 도구의 아주 유용한 데이터 소스이기도 합니다.
...
-->
#### crypto 모듈 내의 UUID 지원

임의의 [RFC 4122](https://www.rfc-editor.org/rfc/rfc4122.txt) 버전 4 UUID 문자열 생성을 위한 새로운 `crypto.randomUUID()` 메서드를 사용 할 수 있습니다.
...
James M Snell이 기여했습니다. [#36729](https://github.com/nodejs/node/pull/36729).
...
-->
#### `AbortController`와 `AbortSignal`을 위한 실험적인 지원

Node.js v14.17.0에 `AbortController`와 `AbortSignal`을 위해 실험적인 부분 지원을 추가했습니다.

두 생성자 모두 전역적으로 `--experimental-abortcontroller`플래그를 통해 활성화됩니다.

덧붙여, 여러 Node.js API에 취소를 위한 `AbortSignal`를 지원하기 위해 업데이트되었습니다.
빌트인 생성자와 함께 위 플래그를 사용하는 것은 의무가 아닙니다. 규격을 준수한 어떠한 대체 서드파티도 호환이 되어야 합니다.

다음 메소드에 `AbortSignal` 지원이 추가되었습니다:
...
* `https.request`
* `http2Session.request`
* `setImmediate`와 `setTimeout`에 프로미스화 변수
...
* **doc**:
  * 폐기 예정이던 기존 URL API의 상태를 legacy로 변경하였습니다. (James M Snell) [#37784](https://github.com/nodejs/node/pull/37784)
  * 안정성 색인에 legacy 상태를 추가했습니다. (James M Snell) [#37784](https://github.com/nodejs/node/pull/37784)
  * 리포트 API의 안정성 상태를 업그레이드했습니다. (Gireesh Punathil) [#35654](https://github.com/nodejs/node/pull/35654)
* **deps**:
  * V8: 애플 실리콘 지원을 위해 다양한 패치를 하위 호환했습니다. (BoHong Li) [#38051](https://github.com/nodejs/node/pull/38051)
  * ICU를 68.1로 업데이트했습니다. (Michaël Zasso) [#36187](https://github.com/nodejs/node/pull/36187)
  * libuv 1.41.0 버전으로 업그레이드했습니다. (Colin Ihrig) [#37360](https://github.com/nodejs/node/pull/37360)
* **http**:
  * http.ClientRequest.getRawHeaderNames()를 추가했습니다. (simov) [#37660](https://github.com/nodejs/node/pull/37660)
  * diagnostics_channel은 요청의 시작과 종료를 보고합니다. (Stephen Belanger) [#34895](https://github.com/nodejs/node/pull/34895)
* **util**:
  * getSystemErrorMap() 함수 구현을 추가했습니다. (eladkeyshawn) [#38101](https://github.com/nodejs/node/pull/38101)
