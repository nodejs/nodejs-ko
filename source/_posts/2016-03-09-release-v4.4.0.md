...
author: Myles Borins
ref: Node v4.4.0 (LTS)
refurl: https://nodejs.org/en/blog/release/v4.4.0/
...
* **deps**:
  - --perf_basic_prof_only_functions라는 새로운 플래그가 추가된 버전의 v8로 업데이트했습니다. (Ali Ijaz Sheikh) [#3609](https://github.com/nodejs/node/pull/3609)
* **http**:
  - HTTP(S) 에이전트가 **유지된(keep alived)** 연결에서 발생하는 에러를 잡을(catch) 수 있습니다. (José F. Romaniello) [#4482](https://github.com/nodejs/node/pull/4482)
* **src**:
  - 빅-엔디안(Big-Endian) 시스템에 대한 지원이 나아졌습니다. (Bryon Leung) [#3410](https://github.com/nodejs/node/pull/3410)
* **tls**:
  - `tls.createSecurePair` 함수에 공통 SSL 옵션을 전달할 수 있는 기능이 추가됐습니다. (Коренберг Марк) [#2441](https://github.com/nodejs/node/pull/2441)
* **tools**:
  - 전달된 독립 파일에 틱 프로세서를 실행하는 `--prof-process` 플래그가 추가됐습니다. (Matt Loring) [#4021](https://github.com/nodejs/node/pull/4021)
...
* **build**:
  - 공백이 포함된 파이썬 경로를 지원합니다. 파이썬이 `c:/Program Files`에 설치된 윈도 사용자에게 특히 유용할 것입니다. (Felix Becker) [#4841](https://github.com/nodejs/node/pull/4841)
* **https**:
  - HTTP/HTTPS 클라이언트 요청이 EPROTO 오류를 던지는(throw) 문제([#3692](https://github.com/nodejs/node/issues/3692))를 해결할 가능성이 있는 수정 사항을 추가했습니다. (Fedor Indutny) [#4982](https://github.com/nodejs/node/pull/4982)
* **installer**:
  - 독립된 틱 로그에 있는 프로파일링 정보를 더 읽기 좋게 만들었습니다. (Matt Loring) [#3032](https://github.com/nodejs/node/pull/3032)
* **npm**:
  - npm 2.14.20으로 업그레이드했습니다. (Kat Marchán) [#5510](https://github.com/nodejs/node/pull/5510)
* **process**:
  - 이벤트 이미터에서 심볼을 지원합니다. ¯\\\_(ツ)\_/¯처럼 작성한 경우에는 심볼이 존재하지 않았습니다. (cjihrig) [#4798](https://github.com/nodejs/node/pull/4798)
* **querystring**:
  - querystring.parse()가 13-22% 가량 빨라졌습니다! (Brian White) [#4675](https://github.com/nodejs/node/pull/4675)
* **streams**:
  - 작은 버퍼 이동에 대한 성능 개선으로 처리량이 5% 증가했습니다. 이번 변경 덕분에 IoT 프로젝트에서 10%의 속도 향상이 관측됐습니다! (Matteo Collina) [#4354](https://github.com/nodejs/node/pull/4354)
* **tools**:
  - eslint를 2.1.0으로 업데이트했습니다. (Rich Trott) [#5214](https://github.com/nodejs/node/pull/5214)
