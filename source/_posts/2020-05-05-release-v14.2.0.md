---
category: articles
title: Node v14.2.0(현재 버전)
author: Michaël Zasso
ref: Node v14.2.0 (Current)
refurl: https://nodejs.org/en/blog/release/v14.2.0
...
#### `assert.CallTracker`를 통한 함수 호출 추적 (실험적)

`assert.CallTracker`는 함수가 호출된 횟수를 추적하고 나중에 확인할 수 있는 새로운
실험적 API입니다. 이것은 `CallTracker` 객체를 생성하고 `calls` 메서드를 사용하여
호출될 때마다 계산되는 래퍼 함수를 생성함으로써 작동합니다. 그런 다음 `verify`
메서드를 사용하여 예상되는 호출 수가 발생했는지 확인할 수 있습니다.
...
function func() {}
// callsfunc()은 tracker.verify() 전에 정확히 두 번만 호출되어야 한다.
const callsfunc = tracker.calls(func, 2);
callsfunc();
callsfunc();

function otherFunc() {}
// 두 번째 파라메터의 기본 값은 `1`이다.
const callsotherFunc = tracker.calls(otherFunc);
callsotherFunc();

// tracker.verify()을 호출하고 tracker.calls()가 정확히 몇 번 호출되었는지 검증한다.
process.on('exit', () => {
  tracker.verify();
});
```

덧붙여, `tracker.report()`는 에러가 있다면 에러의 정보가 담겨있는 배열을 반환합니다.
...
ConorDavenport가 기여하였습니다.[#31982](https://github.com/nodejs/node/pull/31982).
