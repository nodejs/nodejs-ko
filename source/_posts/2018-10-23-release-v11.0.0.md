---
category: articles
title: Node v11.0.0(현재 버전)
author: James M Snell
ref: Node v11.0.0 (Current)
refurl: https://nodejs.org/en/blog/release/v11.0.0
...
Node.js 11.0.0이 출시되었습니다! 이는 내부, 성능, V8 7.0으로 업데이트를 중점으로 개량한 최신의 Node.js 현재 릴리스 라인입니다.
...
* 빌드
 * 더 이상 FreeBSD 10을 지원하지 않습니다. [#22617](https://github.com/nodejs/node/pull/22617)
* `child_process`
  * `windowsHide` 옵션의 기본값이 `true`로 변경되었습니다. [#21316](https://github.com/nodejs/node/pull/21316)
* `console`
  * 타이머가 리셋되지 않았다면 `console.countReset()`은 경고를 출력합니다. [#21649](https://github.com/nodejs/node/pull/21649)
  * 만약 타이머가 존재하면 `console.time()`은 타이머를 리셋하지 않습니다. [#20442](https://github.com/nodejs/node/pull/20442)
* 의존성
  * V8이 7.0으로 업데이트되었습니다. [#22754](https://github.com/nodejs/node/pull/22754)
* `fs`
  * 이제 `fs.read()`메서드는 콜백이 필요합니다. [#22146](https://github.com/nodejs/node/pull/22146)
  * 삭제 예정이었던 `fs.SyncWriteStream` 유틸리티는 삭제되었습니다. [#20735](https://github.com/nodejs/node/pull/20735)
* `http`
  * `http`, `https`, `tls` 모듈은 기본적으로 WHATWG URL 파서를 사용합니다. [#20270](https://github.com/nodejs/node/pull/20270)
* 일반
  * `process.binding()`은 삭제예정입니다. `process.binding()`을 사용하는 코드에서는 이식과 재계산을 할 예정입니다. 만약 지원되는 API의 대안이 없다면, Node.js 깃허브 저장소에 이슈를 생성해주세요. 그럼 적절한 대안에 대한 논의가 있을 것입니다.
  * 실험적으로 구현했던 `queueMicrotask()`가 추가되었습니다. [#22951](https://github.com/nodejs/node/pull/22951)
* 내부
  * 윈도우즈 성능 측정 지원은 삭제되었습니다. [#22485](https://github.com/nodejs/node/pull/22485)
  * `--expose-http2` 명령줄 옵션은 삭제되었습니다. [#20887](https://github.com/nodejs/node/pull/20887)
* 타이머
  * 간격 타이머는 이전 간격에서 에러를 던져도 다시 스케쥴 됩니다. [#20002](https://github.com/nodejs/node/pull/20002)
  * `nextTick` 큐는 매번 그리고 각 타이머 마다 실행됩니다. [#22842](https://github.com/nodejs/node/pull/22842)
* `util`
  * WHATWG의 `TextEncoder`, `TextDecoder`는 전역입니다. [#22281](https://github.com/nodejs/node/pull/22281)
  * `util.inspect()`의 출력 크기는 기본으로 128 MB로 제한됩니다. [#22756](https://github.com/nodejs/node/pull/22756)
  * `NODE_DEBUG`가 `http`나 `http2`에 설정되어 있다면 실행시간 경고가 출력됩니다. [#21914](https://github.com/nodejs/node/pull/21914)
