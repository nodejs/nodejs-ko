---
category: articles
title: Node v7.0.0 (현재 버전)
author: James M Snell
ref: Node v7.0.0 (Current)
refurl: https://nodejs.org/en/blog/release/v7.0.0
...
이번에 릴리스 된 Node.js의 새로운 메이저 버전은 ES6 언어의 기능을 98% 지원하며 신뢰성과 성능이 개선된 V8 5.4를 포함하고 있으며, [WHATWG URL 표준](https://url.spec.whatwg.org/)에 바탕을 둔 실험적인 URL 파서도 포함하고 있습니다.

Node.js v7은 홀수 버전 릴리스이므로 [장기 지원 계획](https://github.com/nodejs/lts)에 따라 2017년 6월까지만 지원됩니다. Node.js v8은 2017년 4월에 출시할 예정입니다.
...
* 중대형 기업과 같이 **안정성**이 필요하거나 프로덕션 환경이 복잡하다면 Node.js v4를 유지하거나 Node.js v6로 업그레이드하세요.
...
-->
다음 릴리스 노트는 호환성 문제가 있을 수 있는 주요 변경 사항을 기술합니다. 이번 버전의 Node.js는 V8 새 버전을 탑재하고 있으므로 기존에 설치된 네이티브 애드온을 다시 컴파일해야 합니다. 그렇지 않으면 네이티브 애드온을 읽어 들일 때 런타임 에러가 발생할 것입니다. 간단하게 `npm rebuild`를 실행하거나 `node_modules`를 삭제한 후 아무것도 없는 상태에서 `npm install`을 실행해도 됩니다.
...
* Buffer
  * 이제 Buffer.byteLength에 잘못된 입력이 전달되면 에러가 발생합니다. [#8946](https://github.com/nodejs/node/pull/8946)
  * new 키워드 없이 Buffer를 호출하는 방법이 폐지 예정(deprecated)되었으며 이 방법을 사용하면 프로세스 경고가 발생합니다. [#8169](https://github.com/nodejs/node/pull/8169)
  * allocUnsafe에 음수를 전달하면 에러가 발생합니다. [#7079](https://github.com/nodejs/node/pull/7079)
* Child Process
  * fork와 execFile 메서드에서 인수의 유효성 검사를 더 엄격하게 합니다. [#7399](https://github.com/nodejs/node/pull/7399)
* Cluster
  * worker.suicide 메서드가 폐지 예정(deprecated)되었으며 호출하면 프로세스 경고가 발생합니다. [#3747](https://github.com/nodejs/node/pull/3747)
* Deps
  * V8 엔진을 5.4.500.36으로 업데이트했습니다. [#8317](https://github.com/nodejs/node/pull/8317) [#8852](https://github.com/nodejs/node/pull/8852)
  [#9253](https://github.com/nodejs/node/pull/9253)
  * `NODE_MODULE_VERSION`을 51로 업데이트했습니다. [#8808](https://github.com/nodejs/node/pull/8808)
* File System
  * 비동기 파일 시스템 메서드에 콜백을 전달하지 않으면 프로세스 경고가 발생합니다. [#7897](https://github.com/nodejs/node/pull/7897)
* Intl
  * Intl.v8BreakIterator 생성자가 폐지 예정(deprecated)되었으며 호출하면 프로세스 경고가 발생합니다. [#8908](https://github.com/nodejs/node/pull/8908)
* Promises
  * 앞으로는 Promise의 거부(rejection)를 반드시 처리해야 합니다(현재는 폐지 예정 상태). 처리하지 않으면 프로세스 경고가 발생합니다. [#8217](https://github.com/nodejs/node/pull/8217)
* Punycode
  * `punycode` 모듈이 폐지 예정(deprecated)되었습니다. [#7941](https://github.com/nodejs/node/pull/7941)
* URL
  * 실험적인 WHATWG URL 파서가 도입되었습니다. [#7448](https://github.com/nodejs/node/pull/7448)
