---
category: articles
title: Node v10.0.0(현재 버전)
author: James M Snell
ref: Node v10.0.0 (Current)
refurl: https://nodejs.org/en/blog/release/v10.0.0
...
Node.js 10.0.0은 노드 재단이 시작된 이래로 7번째 주요 릴리스입니다. 2018년 10월 [Long Term Support 지원](https://github.com/nodejs/release) 브랜치가 될 예정입니다.

특별히 축하하고 싶은것은 실험 단계였던 N-API 네이티브 애드온 API가 실험 단계를 넘어선 것이고, 이 릴리스는 또한 N-API를 완전히 지원하고 [VSCode 확장]( https://marketplace.visualstudio.com/items?itemName=ttd-trace-tools.node-chakracore-time-travel-debugger)을 통해 시간여행(Time-Travel) 기술에 쉽게 접근할 수 있는 [Node-ChakraCore](https://nodejs.org/download/chakracore-release/v10.0.0/)라는 실험 단계의 릴리즈와 통합되어 발표될 것입니다.

아래는 Node-ChakraCore 릴리스에 포함된 주요 기능입니다.

* N-API를 완전히 지원합니다.
* [Visual Studio Code 확장](https://marketplace.visualstudio.com/items?itemName=ttd-trace-tools.node-chakracore-time-travel-debugger)을 통해 시간여행 디버깅을 손쉽게 시작할 수 있도록 합니다.
* generators와 async 함수를 위한 TTD를 지원합니다.
* 검사(Inspector) 프로토콜을 지원합니다.
...
* Assert
  * `assert.fail()`을 하나 이상의 인자와 함께 호출하는 방법은 폐기 되었습니다. [[`70dcacd710`](https://github.com/nodejs/node/commit/70dcacd710)]
  * `assert.ok()`를 인자없이 호출하면 에러가 발생합니다. [[`3cd7977a42`](https://github.com/nodejs/node/commit/3cd7977a42)]
  *  `assert.ifError()`는 `undefined` 혹은 `null`이 아닌경우 인자 값과 함께 에러가 발생합니다. 이 메소드는 참 값과 함께 에러를 발생했었습니다. [[`e65a6e81ef`](https://github.com/nodejs/node/commit/e65a6e81ef)]
  * `assert.rejects()`와 `assert.doesNotReject()` 메소드는 비동기 함수와도 작동하도록 추가되었습니다. [[`599337f43e`](https://github.com/nodejs/node/commit/599337f43e)]
* Async_hooks
  * 오래된 실험상태의 async_hooks API는 제거되었습니다. [[`1cc6b993b9`](https://github.com/nodejs/node/commit/1cc6b993b9)]
* Buffer
  * `node_modules` 폴더 밖에서 `new Buffer()`와 `Buffer()`를 사용하면 실행 시 폐기 경고를 발생합니다. [[`9d4ab90117`](https://github.com/nodejs/node/commit/9d4ab90117)]
  * `Buffer.isEncoding()`은 빈 문자열 등 잘못된 값에 `undefined`를 반환합니다. [[`452eed956e`](https://github.com/nodejs/node/commit/452eed956e)]
  * `Buffer.fill()`은 빈 `Buffer`로 채우길 시도하면 에러가 발생합니다. [[`1e802539b2`](https://github.com/nodejs/node/commit/1e802539b2)]
* Child Process
  * 정의되지 않은 환경변수는 무시됩니다. [[`38ee25e2e2`](https://github.com/nodejs/node/commit/38ee25e2e2)], [[`85739b6c5b`](https://github.com/nodejs/node/commit/85739b6c5b)]
* Console
  * `console.table()`메소드가 추가되었습니다. [[`97ace04492`](https://github.com/nodejs/node/commit/97ace04492)]
* Crypto
  * `crypto.createCipher()`와 `crypto.createDecipher()`메소드는 폐기되었습니다. 대신 `crypto.createCipheriv()`와 `crypto.createDecipheriv()`를 사용해주세요. [[`81f88e30dd`](https://github.com/nodejs/node/commit/81f88e30dd)]
  * `decipher.finaltol()`메소드는 폐기되었습니다. [[`19f3927d92`](https://github.com/nodejs/node/commit/19f3927d92)]
  * `crypto.DEFAULT_ENCODING` 프로퍼티는 폐기되었습니다. [[`6035beea93`](https://github.com/nodejs/node/commit/6035beea93)]
  * `ECDH.convertKey()`메소드가 추가되었습니다. [[`f2e02883e7`](https://github.com/nodejs/node/commit/f2e02883e7)]
  * `crypto.fips` 프로퍼티는 폐기되었습니다. [[`6e7992e8b8`](https://github.com/nodejs/node/commit/6e7992e8b8)]
* Dependencies
  * V8이 6.6으로 업데이트되었습니다. [[`9daebb48d6`](https://github.com/nodejs/node/commit/9daebb48d6)]
  * OpenSSL이 1.1.0h로 업데이트되었습니다. [[`66cb29e646`](https://github.com/nodejs/node/commit/66cb29e646)]
* EventEmitter
  * `EventEmitter.prototype.removeListener()`의 단축 메소드로 `EventEmitter.prototype.off()`메소드가 추가되었습니다.  [[`3bb6f07d52`](https://github.com/nodejs/node/commit/3bb6f07d52)]
* File System
  * `fs/promises` API는 실험적으로 프로미스화된 버전의 `fs`함수를 제공합니다. [[`329fc78e49`](https://github.com/nodejs/node/commit/329fc78e49)]
  * 잘못된 경로 에러는 이제 동기적으로 발생합니다. [[`d8f73385e2`](https://github.com/nodejs/node/commit/d8f73385e2)]
  * `fs.readFile()`메소드는 스레드 풀 고갈을 피하기 위해 분할해 읽습니다. [[`67a4ce1c6e`](https://github.com/nodejs/node/commit/67a4ce1c6e)]
* HTTP
  * HTTP 상태 코드 `100`과 `102-199`의 연산이 향상되었습니다. [[`baf8495078`](https://github.com/nodejs/node/commit/baf8495078)]
  * URL 경로 내의 멀티 바이트 문자는 금지되었습니다. [[`b961d9fd83`](https://github.com/nodejs/node/commit/b961d9fd83)]
* N-API
  * n-api는 더 이상 실험상태가 아닙니다. [[`cd7d7b15c1`](https://github.com/nodejs/node/commit/cd7d7b15c1)]
* Net
  * `'close'`이벤트는 `'end'` 이후 실행됩니다. [[`9b7a6914a7`](https://github.com/nodejs/node/commit/9b7a6914a7)]
* Perf_hooks
  * `PerformanceObserver`클래스는 `AsyncResource`이고 `async_hooks`를 사용해 모니터링할 수 있습니다. [[`009e41826f`](https://github.com/nodejs/node/commit/009e41826f)]
  * 퍼포먼스 이벤트를 위해 추적 이벤트가 발생합니다. [[`9e509b622b`](https://github.com/nodejs/node/commit/9e509b622b)]
  * `performance` API가 간단해졌습니다. [[`2ec6995555`](https://github.com/nodejs/node/commit/2ec6995555)]
  * 퍼포먼스 마일스톤 마크는 추적 이벤트로 실행됩니다. [[`96cb4fb795`](https://github.com/nodejs/node/commit/96cb4fb795)]
* Process
  * 문자열이 아닌 값에 대한 `process.env`는 폐기되었습니다. [[`5826fe4e79`](https://github.com/nodejs/node/commit/5826fe4e79)]
  * `process.assert()`메소드는 폐기되었습니다. [[`703e37cf3f`](https://github.com/nodejs/node/commit/703e37cf3f)]
* REPL
  * REPL은 `--experimental-repl-await` 플레그를 사용할 때 최고 레벨의 await을 실험적으로 지원합니다. [[`eeab7bc068`](https://github.com/nodejs/node/commit/eeab7bc068)]
  * 폐기되었던 "magic mode"는 삭제되었습니다. [[`4893f70d12`](https://github.com/nodejs/node/commit/4893f70d12)]
  * 폐기되었던 `NODE_REPL_HISTORY_FILE` 환경변수는 삭제되었습니다. [[`60c9ad7979`](https://github.com/nodejs/node/commit/60c9ad7979)]
  * 검사 시 프록시 오브젝트는 프록시 오브젝트로 보여집니다. [[`90a43906ab`](https://github.com/nodejs/node/commit/90a43906ab)]
* Streams
  * `'readable'`이벤트는 항상 nextTick과 함께 지연됩니다. [[`1e0f3315c7`](https://github.com/nodejs/node/commit/1e0f3315c7)]
  * 새로운 `pipeline()`메소드가 end-to-data 스트림 파이프라인을 구성하기 위해 지원됩니다. [[`a5cf3feaf1`](https://github.com/nodejs/node/commit/a5cf3feaf1)]
  * 실험적인 지원으로 `stream.Readable`에 비동기 for-await이 추가되었습니다. [[`61b4d60c5d`](https://github.com/nodejs/node/commit/61b4d60c5d)]
* Timers
  * `enroll()`과 `unenroll()`메소드는 폐기되었습니다. [[`68783ae0b8`](https://github.com/nodejs/node/commit/68783ae0b8)]
* TLS
  * `tls.convertNPNProtocols()`메소드는 폐기되었습니다. [[`9204a0db6e`](https://github.com/nodejs/node/commit/9204a0db6e)]
  * NPN(next protocol negotiation)지원을 하지 않습니다. [[`5bfbe5ceae`](https://github.com/nodejs/node/commit/5bfbe5ceae)]
  * `ecdhCurve`의 기본값은 `'auto'`가 되었습니다. [[`af78840b19`](https://github.com/nodejs/node/commit/af78840b19)]
* Trace Events
  * 새로운 `trace_events` 최고 레벨 모듈은 이벤트 카테고리의 추적의 활성화 혹은 비활성화를 허용합니다. [[`da5d818a54`](https://github.com/nodejs/node/commit/da5d818a54)]
* URL
  * WHATWG URL API는 전역 변수로 사용됩니다. [[`312414662b`](https://github.com/nodejs/node/commit/312414662b)]
* Util
  * `util.types.is[…]` 타입 체크가 추가되었습니다. [[`b20af8088a`](https://github.com/nodejs/node/commit/b20af8088a)]
  * `util.inspect()`에 bigint 포메팅이 추가되었습니다. [[`39dc947409`](https://github.com/nodejs/node/commit/39dc947409)]
...
Node.js 10.0.0에서 폐기된 API입니다.

* `assert.fail()`에 하나 이상의 인자를 전달하면 런타임 폐기 경고가 발생합니다. [[`70dcacd710`](https://github.com/nodejs/node/commit/70dcacd710)]
* 폐기되었던 async_hooks API는 수명을 다해 제거되었습니다. [[`1cc6b993b9`](https://github.com/nodejs/node/commit/1cc6b993b9)]
* 여러 Node.js의 내부 의존성에 접근하기 위해 사용된 `require()`는 런타임 폐기 경고가 발생합니다. [[`0e10717e43`](https://github.com/nodejs/node/commit/0e10717e43)]
* `crypto.createCipher()`와 `crypto.createDecipher()`메소드는 문서에서 폐기표시되었습니다.[[`81f88e30dd`](https://github.com/nodejs/node/commit/81f88e30dd)]
* `Decipher.finaltol()`메소드를 사용하면 런타임 폐기 경고가 실행됩니다. [[`19f3927d92`](https://github.com/nodejs/node/commit/19f3927d92)]
* `crypto.DEFAULT_ENCODING`프로퍼티를 사용하면 런타임 폐기 경고가 실행됩니다. [[`6035beea93`](https://github.com/nodejs/node/commit/6035beea93)]
* `Domain`을 전달해 네이티브 에드온이 사용하는 `MakeCallback()`은 런타임 폐기 경고가 실행됩니다?. Use by native addons of the `MakeCallback()` variant that passes a `Domain` will emit a runtime deprecation warning. [[`14bc3e22f3`](https://github.com/nodejs/node/commit/14bc3e22f3)], [[`efb32592e1`](https://github.com/nodejs/node/commit/efb32592e1)]
* 폐기되었던 `net.Server`의 내부 getters/setters의 수명이 다해 제거되었습니다. [[`3701b02309`](https://github.com/nodejs/node/commit/3701b02309)]
* `process.env`를 위한 문자열이 아닌 값의 사용은 문서에서 폐기표시되었습니다. [[`5826fe4e79`](https://github.com/nodejs/node/commit/5826fe4e79)]
* `process.assert()`의 사용은 런타임 폐기 경고가 발생됩니다. [[`703e37cf3f`](https://github.com/nodejs/node/commit/703e37cf3f)]
* 폐기되었던 `NODE_REPL_HISTORY_FILE` 환경 변수는 수명이 다해 제거되었습니다. [[`60c9ad7979`](https://github.com/nodejs/node/commit/60c9ad7979)]
* `timers.enroll()`과 `timers.unenroll()`메소드 사용시 런타임 폐기 경고가 발생합니다. [[`68783ae0b8`](https://github.com/nodejs/node/commit/68783ae0b8)]
* `tls.convertNPNProtocols()`메소드 사용시 런타임 폐기 경고가 발생합니다. NPN을 위한 지원은 Node.js에서 종료되었습니다. [[`9204a0db6e`](https://github.com/nodejs/node/commit/9204a0db6e)]
* `crypto.fips` 프로퍼티는 문서에서 폐기표시되었습니다. [[`6e7992e8b8`](https://github.com/nodejs/node/commit/6e7992e8b8)]
