...
Node.js 10.0.0은 Node.js 재단이 시작된 이래로 7번째 주요 릴리스입니다. 2018년 10월 [장기 지원](https://github.com/nodejs/release) 브랜치가 될 예정입니다.

특별히 축하하고 싶은 것은 실험 단계였던 N-API 네이티브 애드온 API가 실험 단계를 졸업했다는 것입니다. 이 릴리스는 또한 N-API를 완전히 지원하고 [VSCode 확장](https://marketplace.visualstudio.com/items?itemName=ttd-trace-tools.node-chakracore-time-travel-debugger)을 통해 시간여행(Time-Travel) 기술에 쉽게 접근할 수 있는 [Node-ChakraCore](https://nodejs.org/download/chakracore-release/v10.0.0/)라는 실험 단계의 릴리즈와 통합되어 발표될 것입니다.
...
* Assert
  ...
  * 비동기 함수를 위해 `assert.rejects()`와 `assert.doesNotReject()` 메소드를 추가했습니다. [[`599337f43e`](https://github.com/nodejs/node/commit/599337f43e)]
* Async_hooks
...
* `Decipher.finaltol()`메소드를 사용하면 런타임 폐기 경고가 발생합니다. [[`19f3927d92`](https://github.com/nodejs/node/commit/19f3927d92)]
* `crypto.DEFAULT_ENCODING`프로퍼티를 사용하면 런타임 폐기 경고가 발생합니다. [[`6035beea93`](https://github.com/nodejs/node/commit/6035beea93)]
* `Domain`을 전달해 네이티브 에드온이 사용하는 `MakeCallback()`은 런타임 폐기 경고가 발생합니다. [[`14bc3e22f3`](https://github.com/nodejs/node/commit/14bc3e22f3)], [[`efb32592e1`](https://github.com/nodejs/node/commit/efb32592e1)]
