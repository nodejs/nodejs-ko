...
Node.js 10.0.0은 노드 재단이 시작된 이래로 7번째 주요 릴리스입니다. 2018년 10월 [장기 지원](https://github.com/nodejs/release) 브랜치가 될 예정입니다.

특별히 축하하고 싶은 것은 실험 단계였던 N-API 네이티브 애드온 API가 실험 단계를 넘어섰다는 것입니다. 이 릴리스는 또한 N-API를 완전히 지원하고 [VSCode 확장]( https://marketplace.visualstudio.com/items?itemName=ttd-trace-tools.node-chakracore-time-travel-debugger)을 통해 시간여행(Time-Travel) 기술에 쉽게 접근할 수 있는 [Node-ChakraCore](https://nodejs.org/download/chakracore-release/v10.0.0/)라는 실험 단계의 릴리즈와 통합되어 발표될 것입니다.
...
* Assert
  * `assert.fail()`을 하나 이상의 인자와 함께 호출하는 방법은 폐기 예정입니다. [[`70dcacd710`](https://github.com/nodejs/node/commit/70dcacd710)]
  * `assert.ok()`를 인자없이 호출하면 에러가 발생합니다. [[`3cd7977a42`](https://github.com/nodejs/node/commit/3cd7977a42)]
  *  `assert.ifError()`는 `undefined` 혹은 `null`이 아닌 경우 인자 값과 함께 에러가 발생합니다. 이 메소드는 전에는 참 값과 함께 에러를 발생했었습니다. [[`e65a6e81ef`](https://github.com/nodejs/node/commit/e65a6e81ef)]
...
* Buffer
  * `node_modules` 폴더 밖에서 `new Buffer()`와 `Buffer()`를 사용하면 실행 시 폐기 예정 경고를 발생합니다. [[`9d4ab90117`](https://github.com/nodejs/node/commit/9d4ab90117)]
...
* Crypto
  * `crypto.createCipher()`와 `crypto.createDecipher()`메소드는 폐기 예정입니다. 대신 `crypto.createCipheriv()`와 `crypto.createDecipheriv()`를 사용해주세요. [[`81f88e30dd`](https://github.com/nodejs/node/commit/81f88e30dd)]
  * `decipher.finaltol()`메소드는 폐기 예정입니다. [[`19f3927d92`](https://github.com/nodejs/node/commit/19f3927d92)]
  * `crypto.DEFAULT_ENCODING` 프로퍼티는 폐기 예정입니다. [[`6035beea93`](https://github.com/nodejs/node/commit/6035beea93)]
  * `ECDH.convertKey()`메소드가 추가되었습니다. [[`f2e02883e7`](https://github.com/nodejs/node/commit/f2e02883e7)]
  * `crypto.fips` 프로퍼티는 폐기 예정입니다. [[`6e7992e8b8`](https://github.com/nodejs/node/commit/6e7992e8b8)]
* Dependencies
...
* Process
  * 문자열이 아닌 값에 대한 `process.env`는 폐기 예정입니다. [[`5826fe4e79`](https://github.com/nodejs/node/commit/5826fe4e79)]
  * `process.assert()`메소드는 폐기 예정입니다. [[`703e37cf3f`](https://github.com/nodejs/node/commit/703e37cf3f)]
* REPL
  * REPL은 `--experimental-repl-await` 플레그를 사용할 때 최고 레벨의 await를 실험적으로 지원합니다. [[`eeab7bc068`](https://github.com/nodejs/node/commit/eeab7bc068)]
  * 폐기 예정이던 "magic mode"는 삭제되었습니다. [[`4893f70d12`](https://github.com/nodejs/node/commit/4893f70d12)]
  * 폐기 예정이던 `NODE_REPL_HISTORY_FILE` 환경변수는 삭제되었습니다. [[`60c9ad7979`](https://github.com/nodejs/node/commit/60c9ad7979)]
  * 검사 시 프록시 오브젝트는 프록시 오브젝트로 보여집니다. [[`90a43906ab`](https://github.com/nodejs/node/commit/90a43906ab)]
* Streams
  * `'readable'`이벤트는 항상 nextTick으로 실행됩니다. [[`1e0f3315c7`](https://github.com/nodejs/node/commit/1e0f3315c7)]
  * 새로운 `pipeline()`메소드는 비정상 데이터 종료 스트림 파이프라인 생성을 위해 제공됩니다. [[`a5cf3feaf1`](https://github.com/nodejs/node/commit/a5cf3feaf1)]
...
* 폐기 예정이던 async_hooks API는 수명을 다해 제거되었습니다. [[`1cc6b993b9`](https://github.com/nodejs/node/commit/1cc6b993b9)]
* 여러 Node.js의 내부 의존성에 접근하기 위해 사용된 `require()`는 런타임 폐기 경고가 발생합니다. [[`0e10717e43`](https://github.com/nodejs/node/commit/0e10717e43)]
* `crypto.createCipher()`와 `crypto.createDecipher()`메소드는 문서에 폐기 예정 표시했습니다.[[`81f88e30dd`](https://github.com/nodejs/node/commit/81f88e30dd)]
...
* `Domain`을 전달해 네이티브 에드온이 사용하는 `MakeCallback()`은 런타임 폐기 경고가 실행됩니다. [[`14bc3e22f3`](https://github.com/nodejs/node/commit/14bc3e22f3)], [[`efb32592e1`](https://github.com/nodejs/node/commit/efb32592e1)]
* 폐기 예정이던 `net.Server`의 내부 getters/setters의 수명이 다해 제거되었습니다. [[`3701b02309`](https://github.com/nodejs/node/commit/3701b02309)]
* `process.env`를 위한 문자열이 아닌 값의 사용은 문서에 폐기 예정 표시했습니다. [[`5826fe4e79`](https://github.com/nodejs/node/commit/5826fe4e79)]
* `process.assert()`의 사용은 런타임 폐기 경고가 발생됩니다. [[`703e37cf3f`](https://github.com/nodejs/node/commit/703e37cf3f)]
* 폐기 예정이던 `NODE_REPL_HISTORY_FILE` 환경 변수는 수명이 다해 제거되었습니다. [[`60c9ad7979`](https://github.com/nodejs/node/commit/60c9ad7979)]
...
* `crypto.fips` 프로퍼티는 문서에 폐기 예정 표시했습니다. [[`6e7992e8b8`](https://github.com/nodejs/node/commit/6e7992e8b8)]
