...
* **cli**
  * 여러 단어로 이루어진 명령행 옵션에 `_` 를 `-`로 일반화 하였습니다. 즉, `--no_warnings`는 `--no-warnings`와 같은 옵션입니다. [#23020](https://github.com/nodejs/node/pull/23020)
  * `node` 바이너리에 bash 자동완성이 추가되었습니다. bash 자동완성 스크립트를 생성하기 위해서는 `node --completion-bash`를 실행하세요. 결과물은 자동완성이 가능한 소스 파일로 저장될 것입니다. [#20713](https://github.com/nodejs/node/pull/20713)
* **crypto**
  * PEM 레벨 암호화 지원이 추가되었습니다. [#23151](https://github.com/nodejs/node/pull/23151)
  * API 대칭키 쌍 생성이 추가되었습니다. 새로운 매서드 `crypto.generateKeyPair`와 `crypto.generateKeyPairSync`는 공개키와 개인키를 생성할 때 사용됩니다. API는 RSA, DSA, EC를 지원하고 다양한 키 인코딩(PEM과 DER)을 지원합니다. [#22660](https://github.com/nodejs/node/pull/22660)
* **fs**
...
* **http2**
  * ACK `PING`이 아닌 `PING`을 받을 때 마다 `Http2Session`에 `'ping'` 이벤트가 발생하도록 추가했습니다. [#23009](https://github.com/nodejs/node/pull/23009)
...
* **module**
  * `module.createRequireFromPath(filename)`를 추가했습니다. 이 메서드는 파일이름 경로와 관련된 모듈 문제를 해결하기 위해 사용자 정의 필수 함수를 생성하는데 사용될 수 있습니다. [#19360](https://github.com/nodejs/node/pull/19360)
* **process**
  * `Promise`가 resolve될 때마다 사용 될 `'multipleResolves'` 프로세스 이벤트를 추가했습니다. 즉, `Promise` 실행기 안에서 `resolve`와 `reject`함수가 모두 호출될 때 사용됩니다. [#22218](https://github.com/nodejs/node/pull/22218)
* **url**
  * `url.fileURLToPath(url)`와 `url.pathToFileURL(path)`이 추가되었습니다. 이 메서드는 파일 URL과 파일 절대 경로를 정확히 변경할 때 사용할 수 있습니다. [#22506](https://github.com/nodejs/node/pull/22506)
* **util**
  * `util.inspect()`에 `sorted` 옵션을 추가했습니다. 만약 `ture`로 설정하면 객체의 모든 프로퍼티, Set, Map 원소들이 반환 문자열에서 정렬될 것입니다. 만약 함수로 설정한다면 비교 함수로 사용될 것입니다. [#22788](https://github.com/nodejs/node/pull/22788)
  * `util.inspect.custom` 심볼은 `Symbol.for('nodejs.util.inspect.custom')`으로 전역 심볼로 등록되었습니다. [#20857](https://github.com/nodejs/node/pull/20857)
...
* **Windows**
  * 윈도우즈 msi 인스톨러는 네이티브 모듈을 빌드하기 위해 필요한 툴을 자동으로 설치하는 옵션을 제공합니다. [#22645](https://github.com/nodejs/node/pull/22645)
* **Workers**
  * DevTools 프로토콜을 사용하는 Worker를 위한 디버깅 지원이 추가되었습니다. [#21364](https://github.com/nodejs/node/pull/21364)
  * 공개 `inspector` 모듈은 Worker에서 사용 가능합니다. [#22769](https://github.com/nodejs/node/pull/22769)
* **새로운 협업자를 추가했습니다**:
  * [digitalinfinity](https://github.com/digitalinfinity) - Hitesh Kanwathirtha
