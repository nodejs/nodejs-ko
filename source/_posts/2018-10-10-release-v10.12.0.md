---
category: articles
title: Node v10.12.0(현재 버전)
author: Michaël Zasso
ref: Node v10.12.0 (Current)
refurl: https://nodejs.org/en/blog/release/v10.12.0
...
* **assert**
  * 다른 값과 비교하는 값 검증 시 객체 프로퍼티 정렬로 인해 diff 출력이 조금 더 향상되었습니다. [#22788](https://github.com/nodejs/node/pull/22788)
* **cli**
  * 모든 multi-word command-lime 옵션에 `_` 를 `-`로 일반화 하였습니다. 즉, `--no_warnings`는 `--no-warnings`와 같은 옵션입니다. [#23020](https://github.com/nodejs/node/pull/23020)
  * `node` 바이너리에 bash completion이 추가되었습니다. bash completion 스크립트를 생성하기 위해서는 `node --completion-bash`를 실행하세요. 결과물은 completion이 가능한 소스 파일로 저장될 것입니다. [#20713](https://github.com/nodejs/node/pull/20713)
* **crypto**
  * PEM-level 암호화 지원이 추가되었습니다. [#23151](https://github.com/nodejs/node/pull/23151)
  * API 대칭키 쌍 생성이 추가되었습니다. 새로운 매소드 `crypto.generateKeyPair`와 `crypto.generateKeyPairSync`는 공개키와 개인키를 생설할 때 사용됩니다. API는 RSA, DSA, EC를 지원하고 다양한 키 인코딩(PEM과 DER)을 지원합니다. [#22660](https://github.com/nodejs/node/pull/22660)
* **fs**
  * `fs.mkdir`과 `fs.mkdirSync`에 `recursive` 옵션이 추가되었습니다. 만약 이 옵션이 true로 설정되었다면 존재하지 않는 상위 폴더를 자동적으로 생성할 것입니다. [#21875](https://github.com/nodejs/node/pull/21875)
* **http2**
  * `Http2Session`에 언제든 non-ack `PING`을 받을 수 있는 `'ping'` 이벤트가 추가되었습니다. [#23009](https://github.com/nodejs/node/pull/23009)
  * `ORIGIN` 프레임 지원이 추가되었습니다. [#22956](https://github.com/nodejs/node/pull/22956)
  * nghttp2 버전이 1.34.0으로 업데이트 되었습니다. HTTP/2를 이용한 웹 소켓 사용이 가능하도록 한 RFC 8441 확장 연결 프로토콜을 포함합니다. [#23284](https://github.com/nodejs/node/pull/23284)
* **module**
  * `module.createRequireFromPath(filename)`를 추가했습니다. 이 메소드는 파일이름 경로와 관련된 모듈 문제를 해결하기 위해 사용자 정의 필수 함수를 생성하는데 사용될 수 있습니다. [#19360](https://github.com/nodejs/node/pull/19360)
* **process**
  * `Promise`가 해결될 때마다 사용 될 `'multipleResolves'` 프로세스 이벤트를 추가했습니다. 즉, `Promis` 실행기 안에서 `resolve`와 `reject` 함수를 호출할 때 언제든지 사용됩니다. [#22218](https://github.com/nodejs/node/pull/22218)
* **url**
  * `url.fileURLToPath(url)`와 `url.pathToFileURL(path)`이 추가되었습니다. 이 메소드는 파일 URL과 파일 절대 경로를 정확히 변경할 때 사용될 수 있습니다. [#22506](https://github.com/nodejs/node/pull/22506)
* **util**
  * `util.inspect()`에 `sorted` 옵션을 추가했습니다. 만약 `ture`로 설정하면 객체의 모든 프로퍼티, 맵, 셋 원소들이 반환 스트링에서 정렬될 것입니다. 만약 함수로 설정한다면 비교 함수로 사용될 것입니다. [#22788](https://github.com/nodejs/node/pull/22788)
  * `util.inspect.custom` 심볼은 `Symbol.for('nodejs.util.inspect.custom')`으로 글로벌 심볼로 등록되었습니다. [#20857](https://github.com/nodejs/node/pull/20857)
  * `util.format()`에 `BigInt` 숫자 지원이 추가되었습니다. [#22097](https://github.com/nodejs/node/pull/22097)
* **V8 API**
  * 상위 저장소에서 삭제된 여러 V8 C++ API가 폐기예정으로 등록되었습니다. 필요한 곳에 대체 API가 추가되었습니다. [#23159](https://github.com/nodejs/node/pull/23159)
* **Windows**
  * 윈도우즈 msi 인스톨러는 네이티브 모듈을 빌드하기 팰요했던 툴을 자동으로 설치하기 위한 옵션을 제공합니다. [#22645](https://github.com/nodejs/node/pull/22645)
* **Workers**
  * 이미 구현된 개발도구를 사용하는 Workers를 위한 디버깅 지원이 있습니다. [#21364](https://github.com/nodejs/node/pull/21364)
  * 공개 `inspector` 모듈은 Workers에서 사용 가능합니다. [#22769](https://github.com/nodejs/node/pull/22769)
* **새로운 협업자를 추가했습니다**:
  * [digitalinfinity](https://github.com/digitalinfinity) - Hitesh Kanwathirtha
