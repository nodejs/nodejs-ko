---
category: articles
title: Node v8.1.1(현재 버전)
author: Anna Henningsen
ref: Node v8.1.1 (Current)
refurl: https://nodejs.org/en/blog/release/v8.1.1
translator:
...
* **Child processes**
  * `child_process.exec`을 `util.promisify()`로 Promise로 바꾸어 호출하다가 실패한
    오류 결과에서 `stdout`과 `stderr`도 사용할 수 있습니다.
    [[`d66d4fc94c`](https://github.com/nodejs/node/commit/d66d4fc94c)]
    [#13388](https://github.com/nodejs/node/pull/13388)
...
* **Readline**
  * 동일한 입력 스트림에 `readline`을 여러 번 사용하는 코드와 `npm init`을 깨뜨리는
    변경사항을 되돌렸습니다.
    [[`0df6c0b5f0`](https://github.com/nodejs/node/commit/0df6c0b5f0)]
    [#13560](https://github.com/nodejs/node/pull/13560)
