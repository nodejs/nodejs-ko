---
category: articles
title: Node v7.1.0(현재 버전)
author: Evan Lucas
ref: Node v7.1.0 (Current)
refurl: https://nodejs.org/en/blog/release/v7.1.0
...
* **buffer**: 주로 ICU를 사용해 버퍼의 내용을 한 인코딩에서 다른 인코딩으로 변경하기 위해 buffer.transcode를 추가했습니다. (James M Snell) [#9038](https://github.com/nodejs/node/pull/9038)
* **child_process**: IPC 채널을 위해 공개 API를 추가했습니다. (cjihrig) [#9322](https://github.com/nodejs/node/pull/9322)
* **icu**:
  * ICU 58로 업그레이드 함 - 소형 icu (Steven R. Loomis) [#9234](https://github.com/nodejs/node/pull/9234)
  * `process.versions`에 `cldr`, `tz`, `unicode`를 추가했습니다. (Steven R. Loomis) [#9266](https://github.com/nodejs/node/pull/9266)
* **lib**: `String(global) === '\[object global\]'`이 되도록 만들었습니다. (Anna Henningsen) [#9279](https://github.com/nodejs/node/pull/9279)
* **libuv**: 1.10.0로 업그레이드 했습니다. (cjihrig) [#9267](https://github.com/nodejs/node/pull/9267)
* **readline**: icu 기반 문자열의 길이 계산을 사용합니다. (James M Snell) [#9040](https://github.com/nodejs/node/pull/9040)
* **src**:
  * `--preserve-symlinks` 플래그와 같은 효과의 NODE_PRESERVE_SYMLINKS 환경변수 추가 (Marc Udoff) [#8749](https://github.com/nodejs/node/pull/8749)
  * `String#toLocaleUpperCase()`와 `String#toLocaleLowerCase()`를 수정했습니다. (Steven R. Loomis)
