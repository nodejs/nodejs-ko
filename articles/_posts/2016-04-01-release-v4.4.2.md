...
*Update*: 
the version of npm included in this release did not have the correct version string.
이번 릴리스의 npm의 버전은 정확한 버전 문자열을 가지고 있지 않습니다.

As such executing `npm -v` will report `2.15.0` rather than `2.15.1`, which is incorrect. 
`npm -v`와 같은 실행은 `2.15.1`보다 `2.15.0`를 보고해 줍니다. 이것은 잘못되었습니다.

the source code included in this release is in fact the source for `2.15.1`, including the security fix.
보안 수정사항을 포함하고 있는 이번 릴리스의 소스 코드는 사실 `2.15.1` 입니다. 
...
* **https**:
  - Under certain conditions ssl sockets may have been causing a memory leak when keepalive is enabled.
  확실한 상태 하에, ssl 소켓은 keepalive가 활성화 된 상태에서는 메모리릭을 일으키고 있을 것 입니다.
  
  This is no longer the case. (Alexander Penev) [#5713](https://github.com/nodejs/node/pull/5713)
  이것은 더 이상 이 케이스가 아닙니다.  
  
* **lib**:
  - The way that we were internally passing arguments was causing a potential leak.
  우리가 내부적으로 아규먼트(?)를 넘기고 있는 방법은 잠재적 릭을 야기시켰습니다.
  
  By copying the arguments into an array we can avoid this. (Nathan Woltman) [#4361](https://github.com/nodejs/node/pull/4361)
  야규먼트를 배열로 복사함으로써 이것을 회피합니다.
...
* **repl**:
  - Previously if you were using the repl in strict mode the column number would be wrong in a stack trace.
  과거 엄격한 모드에서 repl을 사용했다면 스택 trace에서 column number는 잘못되었습니다. 
  
  This is no longer an issue. (Prince J Wesley) [#5416](https://github.com/nodejs/node/pull/5416)
  이것은 더 이상 이슈가 아닙니다.
