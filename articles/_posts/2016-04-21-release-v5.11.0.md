...
* **Buffer**:
  * 'Buffer.prototype.compare'는 이제 두 버퍼의 하위-범위를 비교할 수 있습니다. (James M Snell) [#5880](https://github.com/nodejs/node/pull/5880)
* **deps**:
...
* **net**:
  * DNS lookup 힌트 통과를 위한 `createConnection()`를 추가 지원합니다. (Colin Ihrig) [#6000](https://github.com/nodejs/node/pull/6000)
* **node**:
  * CLI 옵션 `--eval`과 `--print`은 내장 라이브러리로 사용 가능합니다. (Anna Henningsen) [#6207](https://github.com/nodejs/node/pull/6207)
* **npm**:
  * npm을 3.8.6으로 업그레이드 합니다. [#6153](https://github.com/nodejs/node/pull/6153)
* **repl**:
  * 아무 입력도 없었는데 repl가 마지막 명령을 반복한다면 기본적으로 엔터를 누르세요. 이 행동은 node에 예전부터 있었으며, 의도적으로 삭제하지 않았습니다. (Rich Trott) [#6090](https://github.com/nodejs/node/pull/6090)
* **src**:
