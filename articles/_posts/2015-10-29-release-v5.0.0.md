...
-->
다음 릴리스 노트는 v5가 되면서 추가된 호환성 문제가 있을 수 있는 주요 변경 사항을 기술합니다. 이번 버전의 Node.js는 V8 엔진 새 버전을 탑재하고 있으므로 기존에 설치된 네이티브 애드온을 다시 컴파일해야 합니다. 그렇지 않으면 네이티브 애드온을 읽어들일 때 런타임 에러가 발생할 것입니다. 간단하게 **node_modules** 디렉토리를 제거하고 아무것도 없는 상태로 `npm install`을 실행해도 됩니다.
...
* **console**: _(호환성 깨짐)_ `console.time()`에서 출력되는 값에 소수점 아래 3자리가 추가됩니다 (Michaël Zasso) [#3166](https://github.com/nodejs/node/pull/3166).
* **fs**:
