---
layout: post
title: Node v6.3.0(최신 버전)
author: Jeremiah Senkpiel
ref: Node v6.3.0 (Current)
refurl: https://nodejs.org/en/blog/release/v6.3.0/
translator:
...
* **buffer**: `swap16()`과 `swap32()`를 보완할 `buffer.swap64()`를 추가했습니다. (Zach Bjornson) [#7157](https://github.com/nodejs/node/pull/7157)
* **build**: Node.js를 공유 라이브러리로 빌드하기 위한 새 `configure` 옵션을 추가했습니다. (Stefan Budeanu) [#6994](https://github.com/nodejs/node/pull/6994)
  - 해당 옵션은 `--shared`, `--without-v8-platform`, `--without-bundled-v8`입니다.
* **crypto**: 최상위 인증서를 업데이트했습니다. (Ben Noordhuis) [#7363](https://github.com/nodejs/node/pull/7363)
* **debugger**: `--debug=<address>:<port>`를 통해 서버 주소를 설정할 수 있게 되었습니다. (Ben Noordhuis) [#3316](https://github.com/nodejs/node/pull/3316)
* **npm**: npm을 v3.10.3으로 업그레이드했습니다. (Kat Marchán) [#7515](https://github.com/nodejs/node/pull/7515) & (Rebecca Turner) [#7410](https://github.com/nodejs/node/pull/7410)
* **readline**: readline 생성자에 `prompt` 옵션을 추가했습니다. (Evan Lucas) [#7125](https://github.com/nodejs/node/pull/7125)
* **repl / vm**: `sigint`/`ctrl+c`가 Node.js 인스턴스를 멈추는 대신 무한 루프에서 벗어나게 됩니다. (Anna Henningsen) [#6635](https://github.com/nodejs/node/pull/6635)
* **src**:
  - C++ 공개 API `node::FreeEnvironment`를 추가했습니다. (Cheng Zhao) [#3098](https://github.com/nodejs/node/pull/3098)
  - `require('constants')`를 리팩토링하여 상수를 각각의 모듈에서 바로 접근할 수 있게 되었습니다. (James M Snell) [#6534](https://github.com/nodejs/node/pull/6534)
* **stream**: `readable.read()`의 성능을 최대 70% 향상시켰습니다. (Brian White) [#7077](https://github.com/nodejs/node/pull/7077)
* **timers**: 이제 `setImmediate()`가 특정 상황에서 최대 150% 빨라졌습니다. (Andras) [#6436](https://github.com/nodejs/node/pull/6436)
* **util**: `util.inspect()`에 `breakLength` 옵션을 추가하여 줄과 관련한 객체 출력 형식을 조정할 수 있게 되었습니다. (cjihrig) [#7499](https://github.com/nodejs/node/pull/7499)
* **v8-inspector**: 인스펙터 프로토콜을 통해 Node.js를 디버그할 수 있도록 실험적으로 지원합니다. (Ali Ijaz Sheikh) [#6792](https://github.com/nodejs/node/pull/6792)
  - **Note: 이 기능은 _실험적이며_ 언제든지 변경되거나 삭제될 수 있습니다.**
