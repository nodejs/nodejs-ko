---
layout: post
title: Node v6.2.1(최신 버전)
author: Rod Vagg
ref: Node v6.2.1 (Current)
refurl: https://nodejs.org/en/blog/release/v6.2.1/
...
### 주요 변경사항
* **버퍼**: 이제 'Buffer()'와 'Buffer.alloUnsafe()' 메소드 호출 시 음수 길이를 무시합니다. 이는 애플리케이션 내의 다른 버퍼들에 의해 사용된 메모리 슬랩의 조각들을 노출할 수 있으므로 검사되지 않고 Buffer 생성자 또는 'allocUnsafe()' 메소드에 전달된 사용자 입력에서 발생 가능한 보안 문제(Feross Aboukhadijeh에 의해 보고된)를 수정합니다. 음수 길이들은 이제 더는 Buffer API에 의해 지원되지 않으며 생성자에 대한 사용자 입력은 해가 없어야만 하며 유형을 검사해야만 하는 점을 참고하세요. (Anna Henningsen) [#7051](https://github.com/nodejs/node/pull/7051)
* **npm**: npm을 3.9.3 버전으로 업그레이드 했습니다. (Kat Marchán) [#7030](https://github.com/nodejs/node/pull/7030)
  - [`npm/npm@42d71be`](https://github.com/npm/npm/commit/42d71be2cec674dd9e860ad414f53184f667620d) [npm/npm#12685](https://github.com/npm/npm/pull/12685) semver 식별자 없이 'npm ls <pkg>'를 사용할 때, 'npm ls'는 이름에 의해 매치되는 당신의 트리 안의 아무 패키지들을 건너뛸 수 있습니다만, 그들의 'package.json' 안에 프리 릴리스 버전을 가집니다. ([@zkat](https://github.com/zkat))
  - [`npm/npm@f04e05`](https://github.com/npm/npm/commit/df04e05af1f257a1903372e1baf334c0969fbdbd) [npm/npm#10013](https://github.com/npm/npm/issues/10013) `read-package-tree@5.1.4`: 만약 당신의 'node_modules'가 symlinked 되면 'npm install'이 실패하는 이슈를 수정합니다. ([@iarna](https://github.com/iarna))
  - [`b894413`](https://github.com/npm/npm/commit/b8944139a935680c4a267468bb2d3c3082b5609f) [#12372](https://github.com/npm/npm/issues/12372) 'npm-shrinkwrap.json'에서 중첩된 의존성을 변화시키고 'npm install'이 실행되는 것은 업데이트된 패키지를 얻을 수 없을지도 모릅니다. 이것을 해결합니다. ([@misterbyrne](https://github.com/misterbyrne))
  - 이번 릴리스는 이제 윈도우 운영체제에서 통과(해야만)하는 테스트 슈트인 우리의 윈도우 테스팅 push의 결과물인 'npm@3.9.0'을 포함합니다. 우리는 우리가 Travis처럼 의지할 수 있는 곳으로 AppVeyor를 얻기 위해 노력하고 있습니다.
* **tty**: 이제 OS X의 stdio에 대한 블로킹 모드가 기본입니다. Node.js 6.0.0 버전에서 소개된, libuv 1.9.0 버전의 한 버그 수정은 특히 작은 출력 버퍼를 가진 OS X에서 Node의 non-blocking stdio의 사용에 문제점을 노출했습니다. 이 변화는 OS X의 Node.js 6.0.0 버전 이후 산출 문제를 가져왔던 CLI 애플리케이션들을 수정해야만 합니다. 코어 팀은 지원되는 플랫폼들 사이에서 존재하는 stdio 문제들을 지속해서 해결할 것이며 진행사항은 <https://github.com/nodejs/node/issues/6980>에서 확인할 수 있습니다. (Jeremiah Senkpiel) [#6895](https://github.com/nodejs/node/pull/6895)
* **V8**: V8을 5.0.71.52. 버전으로 업그레이드했습니다. 이번 업그레이드는 Node.js 6.0.0 버전 이후로 node-inspector의 사용자들에 의해 경험되어진 문제들을 해결하는 한 수정사항이 포함되었습니다. 자세한 사항했 <https://github.com/node-inspector/node-inspector/issues/864>에서 확인하세요. (Michaël Zasso) [#6928](https://github.com/nodejs/node/pull/6928)
