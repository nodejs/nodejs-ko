...
* **node**: --no-browser-globals 플래그를 구성합니다. (Fedor Indutny) [#5853](https://github.com/nodejs/node/pull/5853)
* **npm**:
  * 3.8.8 버전으로 업그레이드 하였습니다. 커맨드 라인 인터페이스의 사용자 토큰을 수집하는 서버 구성을 공격자에게 허용하는 HTTP 요청 인증 토큰 사용의 보안 결함을 수정합니다. 이전에는 인증 토큰이 목적지와 상관없이 로그인된 사용자의 CLI로 만들어진 모든 요청과 함께 전송되고 있었습니다. 이번 업데이트에서는 레지스트리나 레지스터의 사용을 막도록 만들어진 요청 토큰만 포함 함으로써 현재 설치의 문제점을 수정합니다. (Forrest L Norvell) [npm/node#6](https://github.com/npm/node/pull/6)
