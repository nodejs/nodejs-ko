---
layout: post
title: Node v5.8.0(안정 버전)
author: Jeremiah Senkpiel
ref: Node v5.8.0 (Stable)
refurl: https://nodejs.org/en/blog/release/v5.8.0/
...
* **child_process**: `send()`에 옵션 파라미터가 추가됐습니다. (cjihrig) [#5283](https://github.com/nodejs/node/pull/5283)
  - 현재 지원하는 옵션은 `keepOpen`뿐인데, 이 옵션이 설정되어 있으면 메시지가 전송된 후에도 기본 소켓을 열어둔 채로 유지합니다.
* **constants**: 이제 `ENGINE_METHOD_RSA`가 올바르게 노출됩니다. (Sam Roberts) [#5463](https://github.com/nodejs/node/pull/5463)
* v5.7.0에서 재발했던 문제 두 가지를 수정했습니다.
  - **http**: 이제 HTTP 클라이언트 콜백 내부의 에러가 올바르게 전파됩니다. (Trevor Norris) [#5591](https://github.com/nodejs/node/pull/5591)
  - **path**: 절대 경로의 정규화 코드를 수정했습니다. (Evan Lucas) [#5589](https://github.com/nodejs/node/pull/5589)
* **repl**: 이제 `start()`에 options 파라미터가 필요하지 않습니다. (cjihrig) [#5388](https://github.com/nodejs/node/pull/5388)
* **util**: `format()`의 성능이 50%에서 300%까지 개선되었습니다. (Evan Lucas) [#5360](https://github.com/nodejs/node/pull/5360)
