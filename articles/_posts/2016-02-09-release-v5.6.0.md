---
layout: post
title: Node v5.6.0(안정 버전)
author: James M Snell
ref: Node v5.6.0 (Stable)
refurl: https://nodejs.org/en/blog/release/v5.6.0/
...
* **http**: 요청과 응답의 HTTP 헤더를 파싱할 때 CVE-2016-2086에 기술된 요청 스머글링(smuggling)을 허용하거나 CVE-2016-2216에 기술된 응답 스플리팅(splitting)을 허용할 수 있는 취약점이 수정됐습니다.
이제 HTTP 헤더 파싱은 수용하는 문자를 제한하는 등 HTTP 스펙에 더 근접하게 조정되었습니다.
* **http-parser**: 2.6.0 버전에서 2.6.1 버전으로 업그레이드 했습니다.
* **npm**: npm을 3.3.12 버전에서o 3.6.0 버전으로 업그레이드 했습니다. (Rebecca Turner) [#4958](https://github.com/nodejs/node/pull/4958)
* **openssl**: 1.0.2e 버전에서 1.0.2f 버전으로 업그레이드 했습니다.
로그잼(Logjam) 공격을 방지하기 위해 TLS 클라이언트는 이제 파라미터가 1024비트보다 짧은 디피-헬만(Diffie-Hellman) 핸드세이크를 거부합니다.
이전에는 768비트까지 허용했었습니다.
