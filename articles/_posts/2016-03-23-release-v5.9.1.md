...
### 주요 변경사항
* **buffer**: 범위를 넘어서서(out-of-bound) 작성을 시도할 때 `RangeError`를 적절히 던집니다. (Matt Loring) [#5605](https://github.com/nodejs/node/pull/5605)
  - `noAssert` 옵션을 사용하지 않을 경우, `write{Float|Double}`에 영향을 줍니다.
* **timers**:
  - 이제 `Timeout` 생성자 이름을 가진 타임아웃 객체를 반환합니다. (Jeremiah Senkpiel) [#5793](https://github.com/nodejs/node/pull/5793)
  - `Immediate` 처리 성능이 20-40% 향상 되었습니다. (Brian White) [#4169](https://github.com/nodejs/node/pull/4169)
* **vm**: contexify v5.9.0에서 재발생한 문제를 수정 했습니다. (Ali Ijaz Sheikh) [#5800](https://github.com/nodejs/node/pull/5800).
