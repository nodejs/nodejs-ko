---
layout: post
title: Node v5.9.1(안정 버전)
author: Jeremiah Senkpiel
ref: Node v5.9.1 (Stable)
refurl: https://nodejs.org/en/blog/release/v5.9.1/
...
### 주요 변경사항
* **buffer**: out-of-bounds 쓰기 `RangeError`s를 적절히 던집니다. (Matt Loring) [#5605](https://github.com/nodejs/node/pull/5605)
  - `noAssert` 옵선을 사용하지 않을 경우, `write{Float|Double}`에 영향을 줍니다.
* **timers**:
  - Returned 타임아웃 객체는 `Timeout` 생성자를 가지게 되었습니다. (Jeremiah Senkpiel) [#5793](https://github.com/nodejs/node/pull/5793)
  - `Immediate` 처리 성능이 20-40% 향상 되었습니다. (Brian White) [#4169](https://github.com/nodejs/node/pull/4169)
* **vm**: 5.9.0 버전의 contextify regression을 수정 했습니다. (Ali Ijaz Sheikh) [#5800](https://github.com/nodejs/node/pull/5800).
