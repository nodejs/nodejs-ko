...
이것은 자격증명의 취약점이 누설되어 npm의 번들 버전을 업그레이드 하는 보안 릴리즈 입니다. 더 자세한 정보는
<http://nodejs.org/en/blog/vulnerability/npm-tokens-leak-march-2016/>에서 볼 수 있습니다.

이번 릴리즈에 포함된 npm의 버전이 올바른 버전 일련이 없음을 유의하세요. 
'npm -v' 명령어 실행에 있어서 '2.15.1'이 아닌 '2.15.0'으로 보여줄 것이며 이는 정확하지 않습니다.
이번 릴리즈에 포함된 소스 코드는 보안 수정을 포함한 실제로 2.15.1 버전에 대한 소스입니다.
...
이번 Node.js의 릴리즈는 SSLv3 또는 더 상위 버전에 사용될 수 있는 이들 목록 안에 포함된 27개의 암호을 완전히 비활성화하는 
